...
def zero_function(*args):
    '''
    This function returns zero, and is used in various places to simplify
    expressions
...
def fortran_substitutions(deriv):
    ...
    # Now we replace ** with PPow so we can later do a string replacement to
    # get just the fortran pow
...
def substitute_rational(expr):
    ...
    This avoids Fortran integer division from ruining our expressions through
    rounding.
    '''
    
    # Search for Rational
    to_sub = [p for p in preorder_traversal(expr) if
              isinstance(p, Rational) and p != 1 and p != -1]
    ...
    # (wildcard matching).
    expr = expr.xreplace(
        dict({old: new for old, new in zip(*(to_sub, new_ex))}))
...
def substitute_dp(expr):
    ...
    This makes all constant declarations type-unambiguous and ensures
...
def substitute_pow(expr):
    ...
    Also replaces instances of pow(x, N/2) with powN(sqrt(x)) because this
    gives better performance.
    (pow(x,y) gets evaluated roughly as exp(y*log(x)), which is costly
    because both log and exp are expensive,
    whereas powN(sqrt(x)) gets evaluated with a faster sqrt routine followed
    by a fast powN)
    '''
    expr_prev = None
    while expr_prev != expr:
        ...
        # no longer matches (because it's looking for x**3+y**2 not pow3(
        # x)+y**2).
        # There may be a clever way to handle this, but I'm just doing an
        # 'iterate-till-it-stops-changing'
        # approach because I know that'll work.
        expr_prev = expr
        
        # Substitutes Pow(x, i/2) for a call to Pow(sqrt(x), i) for all
        # integers i.
        # Then substitutes Pow(x, i) for a call to powI(x) for integers 8 >=
        # i > 0.
...
def py_to_fort(expr):
    ...
    In various places we have used a non-standard expression to ensure that
    sympy gets
    the right behaviour. E.g. we called pow -> PPow and sqrt -> ssqrt. This
    gives us
    more control over the result by preventing sympy from trying to simplify
    things too far.

    At this stage we're generating a string, so we have to undo those
    substitutions
    to get valid fortran code.

    We also take the opportunity to use the fact that MESA has pre-computed
    ln(10)
