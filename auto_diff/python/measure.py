...
def weighted_count_ops(expr_original, verbose=False):
    ...
    if isinstance(
        components[0], Rational
    ):  # Means there's just one operation and it occurs repeatedly.
        parts = {
            str(components[1]): components[0]
        }  # First argument is the number of times the op was used. Second is op name.
    else:
        parts = {}
        for a in components:
            if len(a.args) == 0:
