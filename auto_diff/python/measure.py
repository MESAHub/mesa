...
# The cost of powN is a multiply times ~lnN because it's done by building up
# x^{1,2,4,...}
# and combining these.
...
def weighted_count_ops(expr_original, verbose=False):
    ...
    if isinstance(components[0],
                  Rational):  # Means there's just one operation and it
        # occurs repeatedly.
        parts = {str(components[1]): components[
            0]}  # First argument is the number of times the op was used.
        # Second is op name.
    else:
        parts = {}
        for a in components:
