from sympy import symbols, factorial, diff, DiracDelta, simplify, sign, \
    Derivative
from sympy.simplify.cse_main import cse
from sympy.utilities.iterables import numbered_symbols

from measure import weighted_count_ops
from utils import wrap_element, fortran_substitutions, zero_function, \
    sgn


def unary_generic_chain_rule(auto_diff_type, fixed_length=None):
    '''
    Produces a list of lines of Fortran code to compute all specified
    partial derivatives of a generic unary function z(x),
    where x is itself a function of independent variables (val1, val2, val3,
    ...).
    
    This is done by constructing a series representation of z out to the
    desired order,
    then passing that to the unary_specific_chain_rule method.
    The result is given in terms of the derivatives of z which appear in the
    series expansion.
    '''
    
    # Construct the symbols corresponding to the partial derivatives of z.
    # These are d1z, d2z, ..., dNz, giving dz/dx, d^2 / dx^2, and so on.
    z_symbol_strs = ['z_' + str(p).replace('val1', 'x') for p in
                     auto_diff_type.unary_partials]
    z_symbol_str = ' '.join(z_symbol_strs)
    z_syms = wrap_element(symbols(z_symbol_str, real=True))
    
    def operator(x):
        # Construct z as a power series in terms of its partial derivatives
        # (z_syms) with
        # respect to the x.
...
def binary_generic_chain_rule(auto_diff_type, fixed_length=None):
    ...
    # Construct the symbols corresponding to the partial derivatives of z.
    # These are d1z, d2z, ..., dNz, giving dz/dx, d^2 / dx^2, and so on.
    z_symbol_strs = ['z_' + str(p).replace('val1', 'x').replace('val2', 'y')
                     for p in auto_diff_type.binary_partials]
    z_symbol_str = ' '.join(z_symbol_strs)
    z_syms = wrap_element(symbols(z_symbol_str, real=True))
...
def unary_specific_chain_rule(auto_diff_type, operator, xval=None,
    ...
    '''
    Produces a list of lines of Fortran code to compute all specified
    partial derivatives of a specified unary function z(x),
    where x is itself a function of independent variables (val1, val2, val3,
    ...) and z(x) === operator(x).

    This is done via the power series method.
        1. x is first constructed as a power series in terms of the desired
        partial derivatives.
        2. Then z is constructed as a function of x.
        3. Finally, we use sympy to take symbolic derivatives and extract
        the chain rule result.

    The result is evaluated at the specified xval, if given. Otherwise the
    value of x is left symbolic.
    '''
    
    partials = auto_diff_type.partials
    
    # Construct sympy variables corresponding to the various independent
    # variables.
    # These never appear on the Fortran side, but we keep the naming
    # consistent to correspond to the
    # names in partial_orders.
    # So these are called val1, val2, ..., valN.
    indep_symbol_str = ' '.join(auto_diff_type.partials[0].val_name(i) for i in
                                range(auto_diff_type.num_independent))
    indep_syms = wrap_element(symbols(indep_symbol_str, real=True))
    
    # Construct sympy variables corresponding to the various derivatives
    # dx/d(...).
    # Note that these variable names correspond to the names we'll use on
    # the Fortran side, so
    # we can just directly map sympy expressions to strings and get valid
    # Fortran :-)
    # Hence these are called x%d1val1, x%d2val1, ..., x%d1val2, x%d2val2,
    # ..., x%d1val1_d1val2, ...
    # The first integer in each 'd_val_' block is the number of derivatives,
    # the second is the independent variable those derivatives are with
    # respect to.
    x_symbol_str = ' '.join(
        auto_diff_type.partial_str_in_instance('x', p).replace(':', 'colon')
        for p in partials)
    x_syms = wrap_element(symbols(x_symbol_str, real=True))
    
    # Construct x as a power series in terms of its partial derivatives (
    # sym) with respect to the independent
    # variables (indep).
    x = 0
    ...
    for p in partials:
        ...
        unary_symbol_str = auto_diff_type.partial_str_in_instance('unary',
                                                                  p).replace(
            ':', 'colon')
        ...
        for order, indep in zip(*(p.orders, indep_syms)):
            ...
            d = d.replace(DiracDelta,
                          zero_function)  # Diract Delta is only non-zero in
            # a set of measure zero, so we set it to zero.
            d = d.replace(sign,
                          sgn)  # simplify can do weird things with sign,
            # turning it into the piecewise operator. We want to avoid that
            # so we redirect to a custom sgn function.
            d = d.replace(Derivative,
                          zero_function)  # Eliminates derivatives of the
            # Dirac Delta and Sign, which are non-zero only at sets of
            # measure zero.
            d = d.subs(indep, 0)
    ...
    for d, unary_symbol_str in zip(*(expressions, left_hand_names)):
        derivatives.append(
            unary_symbol_str + ' = ' + str(fortran_substitutions(d)))
...
def binary_specific_chain_rule(auto_diff_type, operator, xval=None, yval=None,
    ...
    '''
    Produces a list of lines of Fortran code to compute all specified
    partial derivatives of a specific
    binary function z(x,y), where x and y are functions of independent
    variables (val1, val2, val3, ...)
    and z === operator(x,y).

    This is done via the power series method.
        1. x and y are first constructed as a power series in terms of the
        desired partial derivatives.
        2. Then z is constructed as a power series in x and y in terms of
        the same partial derivatives.
        3. Finally, we use sympy to take symbolic derivatives and extract
        the chain rule result.

    The result is evaluated at the specified xval and yval, if given.
    Otherwise x and y are left symbolic.
    '''
    ...
    # the second is the independent variable those derivatives are with
    # respect to.
    # Note that unlike in the generic form we include an offset which is the
    # value at which the derivative is evaluated.
    x_symbol_str = ' '.join(
        auto_diff_type.partial_str_in_instance('x', p).replace(':', 'colon')
        for p in partials)
    x_syms = wrap_element(symbols(x_symbol_str, real=True))
    y_symbol_str = ' '.join(
        auto_diff_type.partial_str_in_instance('y', p).replace(':', 'colon')
        for p in partials)
    y_syms = wrap_element(symbols(y_symbol_str, real=True))
    
    # Construct x and y as power series in terms of their partial
    # derivatives (sym)
    ...
    for p in partials:
        # Beginning of Fortran line, for writing out the answer
        binary_symbol_str = auto_diff_type.partial_str_in_instance('binary',
                                                                   p).replace(
            ':', 'colon')
        ...
        for order, indep in zip(*(p.orders, indep_syms)):
            d = diff(d, indep, order)
            d = d.replace(DiracDelta,
                          zero_function)  # Dirac Delta is only non-zero in
            # a set of measure zero, so we set it to zero.
            d = d.replace(sign,
                          sgn)  # simplify can do weird things with sign,
            # turning it into the piecewise operator. We want to avoid that
