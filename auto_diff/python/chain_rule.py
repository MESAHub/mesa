...
def unary_generic_chain_rule(auto_diff_type, fixed_length=None):
    ...
    z_symbol_strs = [
        "z_" + str(p).replace("val1", "x") for p in auto_diff_type.unary_partials
    ]
    ...
    def operator(x):
        ...
        # respect to the x.
        z = sum(
            sym * x ** p.orders[0] / factorial(p.orders[0])
            for sym, p in zip(*(z_syms, auto_diff_type.unary_partials))
        )
        return z

    return unary_specific_chain_rule(
        auto_diff_type, operator, xval=0, fixed_length=fixed_length
    )
...
def binary_generic_chain_rule(auto_diff_type, fixed_length=None):
    ...
    # These are d1z, d2z, ..., dNz, giving dz/dx, d^2 / dx^2, and so on.
    z_symbol_strs = [
        "z_" + str(p).replace("val1", "x").replace("val2", "y")
    ...
    def operator(x, y):
        ...
        z = sum(
            ...
            * x ** p.orders[0]
            * y ** p.orders[1]
            / (factorial(p.orders[0]) * factorial(p.orders[1]))
            for sym, p in zip(*(z_syms, auto_diff_type.binary_partials))
        )
        return z

    return binary_specific_chain_rule(
        auto_diff_type, operator, xval=0, yval=0, fixed_length=fixed_length
    )
...
def unary_specific_chain_rule(auto_diff_type, operator, xval=None, fixed_length=None):
    ...
    indep_symbol_str = " ".join(
        auto_diff_type.partials[0].val_name(i)
        for i in range(auto_diff_type.num_independent)
    )
    ...
    for p in partials:
        ...
        unary_symbol_str = auto_diff_type.partial_str_in_instance("unary", p).replace(
            ":", "colon"
        )
        ...
        for order, indep in zip(*(p.orders, indep_syms)):
            ...
            d = d.replace(
                DiracDelta, zero_function
            )  # Diract Delta is only non-zero in a set of measure zero, so we set it to zero.
            d = d.replace(
                sign, sgn
            )  # simplify can do weird things with sign, turning it into the piecewise operator. We want to avoid that so we redirect to a custom sgn function.
            d = d.replace(
                Derivative, zero_function
            )  # Eliminates derivatives of the Dirac Delta and Sign, which are non-zero only at sets of measure zero.
            d = d.subs(indep, 0)
            ...
            if "colon" in str(sub_expr):  # Intermediate is an array type.
                ...
                for s in sub_expr.free_symbols:
                    if s in is_array:
                        if is_array[s]:
                            is_array[sym] = (
                                True  # Intermediate is defined in terms of an array type and so is an array type.
                            )

    # Write the lines defining the sub-expressions
    for sym, sub_expr in common_sub_expressions[0][
        ::-1
    ]:  # Have to go in reverse order because we're prepending.
        if is_array[sym]:  # # Intermediate is an array type.
            if fixed_length is None:
                sym_str = (
                    str(sym) + "(1:x%n)"
                )  # All of these expressions have x defined as an auto_diff type, so it has an %n property.
            else:
...
def binary_specific_chain_rule(
    auto_diff_type, operator, xval=None, yval=None, fixed_length=None
):
    """
    ...
    for p in partials:
        ...
        for order, indep in zip(*(p.orders, indep_syms)):
            ...
            d = d.replace(
                DiracDelta, zero_function
            )  # Dirac Delta is only non-zero in a set of measure zero, so we set it to zero.
            d = d.replace(
                sign, sgn
            )  # simplify can do weird things with sign, turning it into the piecewise operator. We want to avoid that so we redirect to a custom sgn function.
            d = d.replace(
