...
from chain_rule import (
    unary_generic_chain_rule,
    binary_generic_chain_rule,
    unary_specific_chain_rule,
    binary_specific_chain_rule,
)
...
class AutoDiffType:
    def __init__(self, name, array, array_length, partials):
        ...
        self.partials = sorted(
            list(partials), key=lambda p: [p.net_order, tuple(-o for o in p.orders)]
        )
        ...
        for p in self.partials:
            ...
            if "Array" in p_name:
                if self.array_length is None:
                    ...
                    variables.append(
                        "real(dp) :: " + p_name + "(" + str(self.array_length) + ")"
                    )
            else:
    ...
    def assignment_routine_from_real_dp(self):
        ...
        routine_body = ["this%val = other"] + [
            "this%" + str(p) + " = 0_dp" for p in self.partials if p.net_order > 0
        ]
    ...
    def comparison_function_with_self(self, operator_name, operator):
        ...
        return FortranFunction(
            function_name, function_arguments, function_result, function_body
        )
    ...
    def comparison_function_with_real_dp(self, operator_name, operator):
        ...
        first_function = FortranFunction(
            function_name, function_arguments, function_result, function_body
        )
        ...
        second_function = FortranFunction(
            function_name, function_arguments, function_result, function_body
        )
    ...
    def generic_unary_operator_function(self):
        ...
        function_arguments = function_arguments + [
            ("z_" + str(p).replace("val1", "x"), "real(dp)", "in")
            for p in self.unary_partials
        ]
        function_result = ("unary", self.declare_name(ref="x"))
        function_body, function_declarations = unary_generic_chain_rule(
            self, fixed_length=self.array_length
        )
    ...
    def generic_binary_operator_function(self):
        ...
        function_arguments = function_arguments + [
            ("z_" + str(p).replace("val1", "x").replace("val2", "y"), "real(dp)", "in")
            for p in self.binary_partials
        ]
        function_result = ("binary", self.declare_name(ref="x"))
        function_body, function_declarations = binary_generic_chain_rule(
            self, fixed_length=self.array_length
        )
        function_body = function_declarations + function_body
    ...
    def specific_unary_operator_function(self, operator_name, operator):
        ...
        function_arguments = [("x", self.declare_name(ref="*"), "in")]
        function_result = ("unary", self.declare_name(ref="x"))
        function_body, function_declarations = unary_specific_chain_rule(
            self, operator, fixed_length=self.array_length
        )
        function_body = function_declarations + function_body
    ...
    def specific_binary_operator_function_self(self, operator_name, operator):
        ...
        function_result = ("binary", self.declare_name(ref="x"))
        function_body, function_declarations = binary_specific_chain_rule(
            self, operator, fixed_length=self.array_length
        )
        function_body = function_declarations + function_body
    ...
    def specific_binary_operator_function_real_dp(self, operator_name, operator):
        ...
        function_result = ("unary", self.declare_name(ref="x"))
        function_body, function_declarations = unary_specific_chain_rule(
            self, unary_operator, fixed_length=self.array_length
        )
        function_body = function_declarations + function_body
        function_1 = FortranFunction(
            function_name, function_arguments, function_result, function_body
        )
    ...
    def specific_binary_operator_function_int(self, operator_name, operator):
        ...
        function_body = (
            ["real(dp) :: y_dp"] + function_declarations + ["y_dp = y"] + function_body
        )
        function_1 = FortranFunction(
            function_name, function_arguments, function_result, function_body
        )
        ...
        function_body = (
            ["real(dp) :: y_dp"] + function_declarations + ["y_dp = z"] + function_body
        )
