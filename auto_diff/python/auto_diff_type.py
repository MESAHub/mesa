...
from chain_rule import unary_generic_chain_rule, binary_generic_chain_rule, unary_specific_chain_rule, binary_specific_chain_rule
...
class AutoDiffType:
	...
	def declare_name(self, ref=None):
		if (not self.array) or (not self.array_length is None): # Either no array type or else it's a fixed length.
	...
	def assignment_routine_from_self(self):
		...
		routine_arguments = [('this', self.declare_name(ref='*'), 'out'), ('other', self.declare_name(ref='this'), 'in')]
	...
	def assignment_routine_from_real_dp(self):
		...
		routine_arguments = [('this', self.declare_name(ref='*'), 'out'), ('other', 'real(dp)', 'in')]
		routine_body = ['this%val = other'] + ['this%' + str(p) + ' = 0_dp' for p in self.partials if p.net_order > 0]
	...
	def assignment_routine_from_int(self):
		...
		routine_arguments = [('this', self.declare_name(ref='*'), 'out'), ('other', 'integer', 'in')]
		routine_body = ['this%val = other'] + ['this%' + str(p) + ' = 0_dp' for p in self.partials if p.net_order > 0]
	...
	def comparison_function_with_self(self, operator_name, operator):
		...
		function_arguments = [('this', self.declare_name(ref='*'), 'in'), ('other', self.declare_name(ref='this'), 'in')]
	...
	def comparison_function_with_real_dp(self, operator_name, operator):
		...
		function_arguments = [('this', self.declare_name(ref='*'), 'in'), ('other', 'real(dp)', 'in')]
		function_result = ('z', 'logical')
		function_body = ['z = (this%val ' + operator + ' other)']

		first_function = FortranFunction(function_name, function_arguments, function_result, function_body)

		function_name = operator_name + '_' + 'real_dp' + '_' + self.name 
		function_arguments = [('this', 'real(dp)', 'in'), ('other', self.declare_name(ref='*'), 'in')]
		function_result = ('z', 'logical')
		function_body = ['z = (this ' + operator + ' other%val)']

		second_function = FortranFunction(function_name, function_arguments, function_result, function_body)
	...
	def comparison_function_with_int(self, operator_name, operator):
		...
		function_arguments = [('this', self.declare_name(ref='*'), 'in'), ('other', 'integer', 'in')]
		function_result = ('z', 'logical')
		function_body = ['z = (this%val ' + operator + ' other)']
		...
		function_arguments = [('this', 'integer', 'in'), ('other', self.declare_name(ref='*'), 'in')]
		function_result = ('z', 'logical')
		function_body = ['z = (this ' + operator + ' other%val)']
