...
! version of 1995/08
   ...
   subroutine interp_rgbi3p_db(md, nxd, nyd, xd, yd, zd, nip, xi, yi, zi, ierr, wk)
      ...
      call do_rgbi3p_db(md, nxd, nyd, xd, yd, zd, nip, xi, yi, zi, ierr, wk)
   end subroutine interp_rgbi3p_db

   ! interpolate surface for given grid in x and y
   subroutine interp_rgsf3p_db(md, nxd, nyd, xd, yd, zd, nxi, xi, nyi, yi, zi, ierr, wk)
      ...
      integer, intent(out) :: ierr
      call do_rgsf3p_db(md, nxd, nyd, xd, yd, zd, nxi, xi, nyi, yi, zi, ierr, wk)
...
! these routines are exactly the same except they are real(dp).
   ...
   subroutine interp_cshep2_db(n, x, y, f, nc, nw, nr, lcell, lnext, xmin, ymin, dx, dy, rmax, rw, a, ierr)
      ...
      call do_cshep2_db(n, x, y, f, nc, nw, nr, lcell, lnext, xmin, ymin, dx, dy, rmax, rw, a, ierr)
   end subroutine interp_cshep2_db

   ! see cs2val_sg for documentation details.
   real(dp) function interp_cs2val_db(px, py, n, x, y, f, nr, lcell, lnext, xmin, ymin, dx, dy, rmax, rw, a, ierr)
      ...
      interp_cs2val_db = do_cs2val_db(px, py, n, x, y, f, nr, lcell, lnext, xmin, ymin, dx, dy, rmax, rw, a, ierr)
   end function interp_cs2val_db

   ! see cs2grd_sg for documentation details.
   subroutine interp_cs2grd_db(px, py, n, x, y, f, nr, lcell, lnext, xmin, ymin, dx, dy, rmax, rw, a, c, cx, cy, ierr)
      integer, intent(in) :: n, nr, lcell(nr, nr), lnext(n)
      real(dp), intent(in) :: px, py, x(n), y(n), f(n), xmin, ymin, dx, dy, rmax, rw(n), a(9, n)
      real(dp), intent(out) :: c, cx, cy
      integer, intent(out) :: ierr
      call do_cs2grd_db(px, py, n, x, y, f, nr, lcell, lnext, xmin, ymin, dx, dy, rmax, rw, a, c, cx, cy, ierr)
...
! detailed documentation can be found in interp_2d_lib_sg.f
   ...
   subroutine interp_evbicub_db(xget, yget, x, nx, y, ny, ilinx, iliny, f1, inf2, ict, fval, ierr)
   ...
   subroutine interp_mkspline_db(x, nx, fspl, ibcxmin, bcxmin, ibcxmax, bcxmax, ilinx, ierr)
      ...
      integer, intent(in) :: ilinx                     ! ilinx=1 => assume x evenly spaced
      integer, intent(out) :: ierr                     ! error code =0 ==> no error
      call mkspline_db(x, nx, fspl, ibcxmin, bcxmin, ibcxmax, bcxmax, ilinx, ierr)
...
! use interp_evbipm_db to evaluate it
   ...
   subroutine interp_mkbipm_db(x, nx, y, ny, f1, nf2, ierr)
   ...
   subroutine interp_evbipm_db(xget, yget, x, nx, y, ny, f1, nf2, z, ierr)
