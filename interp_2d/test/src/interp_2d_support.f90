...
contains
   ...
   subroutine setup_to_interp_2D_db(bicub_flag)
      ...
      if (bicub_flag) then
         ...
         call interp_mkbicub_db(xpts_db, num_xpts, ypts_db, num_ypts, f_db1, nf2, &
                                ibcxmin, bcxmin, ibcxmax, bcxmax, ibcymin, bcymin, ibcymax, bcymax, ilinx, iliny, ier)
      else
   ...
   subroutine eval_2D_interp_db(bicub_flag, x, y, z, dz_dx, dz_dy)
      ...
      if (bicub_flag) then
         ...
         call interp_evbicub_db(x, y, xpts_db, num_xpts, ypts_db, num_ypts, ilinx, iliny, f_db1, nf2, ict, fval, ier)
         z = fval(1)
         dz_dx = fval(2)
         dz_dy = fval(3)
      else
         call interp_evbipm_db(x, y, xpts_db, num_xpts, ypts_db, num_ypts, f_db1, nf2, z, ier)
         dz_dx = 0
         dz_dy = 0
      end if
   ...
   subroutine eval_2D_interp_sg(bicub_flag, x, y, z, dz_dx, dz_dy)
      ...
      if (bicub_flag) then
         ...
         ict(5) = 0  ! skip d2f_dy2
         ict(6) = 0  ! skip d2f_dx_dy
         call interp_evbicub_sg(x, y, xpts_sg, num_xpts, ypts_sg, num_ypts, ilinx, iliny, f_sg1, nf2, ict, fval, ier)
         z = fval(1)
         dz_dx = fval(2)
         dz_dy = fval(3)
      else
         call interp_evbipm_sg(x, y, xpts_sg, num_xpts, ypts_sg, num_ypts, f_sg1, nf2, z, ier)
         dz_dx = 0
         dz_dy = 0
      end if
