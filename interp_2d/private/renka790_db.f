      ...
      SUBROUTINE do_CSHEP2_db (N,X,Y,F,NC,NW,NR, LCELL,LNEXT,XMIN,
...
C            fined in Subroutine STORE2_db.  A rectangle con-
...
C               with cells.  Refer to Subroutine STORE2_db.
C
C       LNEXT = Array of length N containing next-node
C               indexes.  Refer to Subroutine STORE2_db.
C
...
C
C Modules required by CSHEP2:  GETNP2_db, GIVENS_db, ROTATE_db,
C                                SETUP2_db, STORE2_db
C
...
C IERR =       Error flag for the call to Subroutine STORE2_db
C IP1 =        I+1
C IRM1 =       IROW-1
...
C                weights for C(K) (see Subroutine SETUP2_db)
...
C                mined by subroutine GIVENS_db
...
C
   21 IER = 1
      RETURN
C
C Duplicate nodes were encountered by GETNP2_db.
C
   22 IER = 2
      RETURN
C
...
C
   23 XMIN = XMN
      ...
      DOUBLE PRECISION FUNCTION do_CS2VAL_db (PX,PY,N,X,Y,F,NR,
...
C CS2HES_db may be called to compute a value, first partial
...
C            Refer to Subroutine STORE2_db.  NR .GE. 1.
C
C       LCELL = NR by NR array of nodal indexes associated
C               with cells.  Refer to Subroutine STORE2_db.
C
...
C
    6 do_CS2VAL_db = 0.
      ...
      SUBROUTINE do_CS2GRD_db (PX,PY,N,X,Y,F,NR,LCELL,LNEXT,XMIN,
...
C
    7 C = 0.
      ...
      RETURN
      END
      SUBROUTINE CS2HES_db (PX,PY,N,X,Y,F,NR,LCELL,LNEXT,XMIN,
...
C
C Modules required by CS2HES_db:  None
C
C Intrinsic functions called by CS2HES_db:  INT, SQRT
C
C***********************************************************
C
      INTEGER I, IMAX, IMIN, J, JMAX, JMIN, K, KP
...
C
    7 C = 0.
      ...
      RETURN
      END
      SUBROUTINE GETNP2_db (PX,PY,X,Y,NR,LCELL,LNEXT,XMIN,YMIN,
     .                   DX,DY, NP,DSQ)
...
C Subroutine STORE2_db, this subroutine uses the cell method to
...
C from their values on output from Subroutine STORE2_db.
C
C On output:
C
...
C             NP, or 0 if NP = 0.
C
C Modules required by GETNP2_db:  None
C
C Intrinsic functions called by GETNP2_db:  ABS, INT, SQRT
C
C***********************************************************
C
...
C (transposed).  Subroutine ROTATE_db may be called to apply
...
C       S = +/-(B/R) or 0 if R = 0.
C
C Modules required by GIVENS_db:  None
C
C Intrinsic functions called by GIVENS_db:  ABS, SQRT
C
C***********************************************************
C
...
C             altered if N < 1).
C
C Modules required by ROTATE_db:  None
C
C***********************************************************
C
      INTEGER I
...
C             gression matrix.
C
C Modules required by SETUP2_db:  None
C
C Intrinsic function called by SETUP2_db:  SQRT
C
C***********************************************************
C
      INTEGER I
...
C
    ...
    2   CONTINUE
      RETURN
      END
      SUBROUTINE STORE2_db (N,X,Y,NR, LCELL,LNEXT,XMIN,YMIN,DX,
     .                   DY,IER)
      INTEGER N, NR, LCELL(NR,NR), LNEXT(N), IER
...
C
C Modules required by STORE2_db:  None
C
C Intrinsic functions called by STORE2_db:  DBLE, INT
C
C***********************************************************
C
...
C
      IER = 0
      !write(*,*) 'STORE2_db -- no errors encountered'
      RETURN
C
C Invalid input parameter.
C
