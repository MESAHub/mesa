...
C            fined in Subroutine STORE2_sg.  A rectangle con-
...
C               with cells.  Refer to Subroutine STORE2_sg.
C
C       LNEXT = Array of length N containing next-node
C               indexes.  Refer to Subroutine STORE2_sg.
C
...
C
C Modules required by CSHEP2:  GETNP2_sg, GIVENS_sg, ROTATE_sg,
C                                SETUP2_sg, STORE2_sg
C
...
C IERR =       Error flag for the call to Subroutine STORE2_sg
C IP1 =        I+1
C IRM1 =       IROW-1
...
C                weights for C(K) (see Subroutine SETUP2_sg)
...
C                mined by subroutine GIVENS_sg
...
C
   21 IER = 1
      RETURN
C
C Duplicate nodes were encountered by GETNP2_sg.
C
   22 IER = 2
      RETURN
C
...
C CS2HES_sg may be called to compute a value, first partial
...
C            Refer to Subroutine STORE2_sg.  NR .GE. 1.
C
C       LCELL = NR by NR array of nodal indexes associated
C               with cells.  Refer to Subroutine STORE2_sg.
C
...
C
C Modules required by CS2HES_sg:  None
C
C Intrinsic functions called by CS2HES_sg:  INT, SQRT
C
C***********************************************************
C
      INTEGER I, IMAX, IMIN, J, JMAX, JMIN, K, KP
...
C from their values on output from Subroutine STORE2_sg.
C
C On output:
C
...
C             NP, or 0 if NP = 0.
C
C Modules required by GETNP2_sg:  None
C
C Intrinsic functions called by GETNP2_sg:  ABS, INT, SQRT
C
C***********************************************************
C
...
C (transposed).  Subroutine ROTATE_sg may be called to apply
...
C       S = +/-(B/R) or 0 if R = 0.
C
C Modules required by GIVENS_sg:  None
C
C Intrinsic functions called by GIVENS_sg:  ABS, SQRT
C
C***********************************************************
C
      REAL AA, BB, R, U, V
...
C             altered if N < 1).
C
C Modules required by ROTATE_sg:  None
C
C***********************************************************
C
      INTEGER I
...
C             gression matrix.
C
C Modules required by SETUP2_sg:  None
C
C Intrinsic function called by SETUP2_sg:  SQRT
C
C***********************************************************
C
      INTEGER I
...
C
C Modules required by STORE2_sg:  None
C
C Intrinsic functions called by STORE2_sg:  DBLE, INT
C
C***********************************************************
C
