...
! Format statements for error messages
 ...
 9020 FORMAT (1X,/,'*** RGBI3P Error 3: Identical XD values or',   ' XD values out of sequence',/,'    IX =',I6,',  XD(IX) =',   E11.3)
 9030 FORMAT (1X,/,'*** RGBI3P Error 4: Identical YD values or',   ' YD values out of sequence',/,'    IY =',I6,',  YD(IY) =',   E11.3)
...
! Format statements for error messages
 ...
 9020 FORMAT (1X,/,'*** RGSF3P_db Error 3: Identical XD values or',   ' XD values out of sequence',/,'    IX =',I6,',  XD(IX) =',   E11.3)
 9030 FORMAT (1X,/,'*** RGSF3P_db Error 4: Identical YD values or',   ' YD values out of sequence',/,'    IY =',I6,',  YD(IY) =',   E11.3)
...
! Statement Function definitions
      ...
      Z3F(XX1,XX2,XX3,ZZ0,ZZ1,ZZ2) = ((ZZ2-ZZ0)* (XX3-XX1)/XX2-                                (ZZ1-ZZ0)* (XX3-XX2)/XX1)*                                (XX3/ (XX2-XX1)) + ZZ0
!     ..
! Calculation
...
! Selects necessary grid points in the x direction.
                  ...
                  IX2 = IX0 + IDLT(2,IPEX)
                  IX3 = IX0 + IDLT(3,IPEX)
                  if ((IX1 < 1) .or. (IX2 < 1) .or. (IX3 < 1) .or. (IX1 > NX0) .or. (IX2 > NX0) .or. (IX3 > NX0)) GOTO 10
...
! Selects necessary grid points in the y direction.
                  ...
                  IY2 = IY0 + IDLT(2,IPEY)
                  IY3 = IY0 + IDLT(3,IPEY)
                  if ((IY1 < 1) .or. (IY2 < 1) .or. (IY3 < 1) .or. (IY1 > NY0) .or. (IY2 > NY0) .or. (IY3 > NY0)) GOTO 20
...
! direction
              do 40 IPEX = 1,4
                  ...
                  IX2 = IX0 + IDLT(2,IPEX)
                  IX3 = IX0 + IDLT(3,IPEX)
                  if ((IX1 < 1) .or. (IX2 < 1) .or. (IX3 < 1) .or. (IX1 > NX0) .or. (IX2 > NX0) .or. (IX3 > NX0)) GOTO 40
...
! direction
                  do 30 IPEY = 1,4
                      ...
                      IY2 = IY0 + IDLT(2,IPEY)
                      IY3 = IY0 + IDLT(3,IPEY)
                      if ((IY1 < 1) .or. (IY2 < 1) .or. (IY3 < 1) .or. (IY1 > NY0) .or. (IY2 > NY0) .or. (IY3 > NY0)) GOTO 30
...
! the coefficient of the bicubic polynomial.
                      ...
                      PEZXY = CX1* (CY1*DZXY11+CY2*DZXY12+CY3*DZXY13) + CX2* (CY1*DZXY21+CY2*DZXY22+CY3*DZXY23) + CX3* (CY1*DZXY31+CY2*DZXY32+CY3*DZXY33)
! Calculates the volatility factor and distance factor in the x
! and y directions for the primary estimate of zxy.
                      ...
                      SXYY = SX*SYY
                      SXXYY = SXX*SYY
                      SXYZ = X1* (Y1*Z11+Y2*Z12+Y3*Z13) + X2* (Y1*Z21+Y2*Z22+Y3*Z23) + X3* (Y1*Z31+Y2*Z32+Y3*Z33)
                      ...
                      VOLF = DZ00**2 + DZ01**2 + DZ02**2 + DZ03**2 + DZ10**2 + DZ11**2 + DZ12**2 + DZ13**2 + DZ20**2 + DZ21**2 + DZ22**2 + DZ23**2 + DZ30**2 + DZ31**2 + DZ32**2 + DZ33**2
                      DISF = SXX*SYY
! Calculates EPSLN.
                      EPSLN = (Z00**2+Z01**2+Z02**2+Z03**2+Z10**2+ Z11**2+Z12**2+Z13**2+Z20**2+Z21**2+Z22**2+ Z23**2+Z30**2+Z31**2+Z32**2+Z33**2)* 1.0E-12
...
!     ..
!     .. Local Scalars ..
      double precision             :: A,B,C,D,DX,DXSQ,DY,DYSQ,P00,P01,P02,P03,P10,P11, P12,P13,P20,P21,P22,P23,P30,P31,P32,P33,Q0,Q1,Q2,   Q3,U,V,X0,XII,Y0,YII,Z00,Z01,Z0DX,Z0DY,Z10,Z11,   Z1DX,Z1DY,ZDXDY,ZII,ZX00,ZX01,ZX0DY,ZX10,ZX11,   ZX1DY,ZXY00,ZXY01,ZXY10,ZXY11,ZY00,ZY01,ZY0DX,   ZY10,ZY11,ZY1DX
