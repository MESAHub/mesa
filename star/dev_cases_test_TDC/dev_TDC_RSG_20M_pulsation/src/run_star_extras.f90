...
call final()
      ...
      subroutine data_for_extra_profile_columns(id, n, nz, names, vals, ierr)
       ...
       ! write (*,*) "vesc(k=1)[km/s] = ", vals(1,1)/1d5
       ! write (*,*) "v div vesc(k=1) = ", vals(1,2)
...
! routine for relaxation rotation
!
...
!         end if
      ...
      subroutine my_before_struct_burn_mix(id, dt, res)
         ...
         if(s% u_flag) then
            call star_read_controls(id, 'inlist_hydro_on', ierr)
            !s% q_for_v_drag_full_off = 1d0 - s% q(2) !0.98d0
            !s% q_for_v_drag_full_on = 1d0 - s% q(1)!0.99d0
            if (s% xtra(x_time_start_pulse) > 0d0) then
               ...
               if (.not. s% lxtra(lx_using_bb_bcs)) then
                  ...
                  s% max_timestep = max_dt_during_pulse ! always !1d99
               end if
               !s% max_q_for_convection_with_hydro_on = 1d0 - s% q(20)!0.999d0
               do k = s% nz, 1, -1
                  v_esc = sqrt(2*s% cgrav(k)*s% m(k)/(s% r(k)))
                  if (s% u(k) > 4d0*v_esc) then ! try 1, 4 in farmer inlist
                ! maybe add check for regions above being above vesc
                 ! write(*,*) 'K'
                     exit
                  end if
               end do
            ...
            !vsurf_for_fixed_bc = sqrt(2*s% cgrav(1)*s% m(1)/(s% r(1))) ! vesc
            do k=1, s% nz
            ...
            if (s% xh(s% i_u,1) >= 1d5*vsurf_for_fixed_bc) then
               ...
               s% use_momentum_outer_BC = .false.   ! wait what?
...
!            end if
         ...
         if(s% ixtra(ix_steps_since_relax) < 50 &
               ...
               .or. s% u_flag) then
            ! check for added stability from ml.
      ...
      integer function extras_finish_step(id)
         ...
         !    end do
         ! end if
         ! make sure vsurf is correct.
         !vsurf_for_fixed_bc = sqrt(2*s% cgrav(1)*s% m(1)/(s% r(1)))
         vesc_surf = sqrt(2*s% cgrav(1)*s% m(1)/(s% r(1)))
         !check_avg_v_div_vesc = 0
         if (remove_extended_layers .and. s% u_flag &
            .and. s% u(1) > vesc_surf) then
            ! .and. s% log_surface_radius > 4.0d0) then
            do k = 1, s% nz
                !check_avg_v_div_vesc = check_avg_v_div_vesc + s% dm(k)*s% u(k)/sqrt(2*s% cgrav(k)*s% m(k)/(s% r(k)))
               if (log10(s% r(k)/Rsun) > 4.0d0) then
                !if (check_avg_v_div_vesc < 0.999d0) then
                !vesc_for_cell = sqrt(2*s% cgrav(k)*s% m(k)/(s% r(k)))
                !write (*,*) "u(k)", s% u(k)
                !write (*,*) "vesc(k)", vesc_for_cell
            
!                if (s% u(k)/vesc_for_cell < 2d0) then
            ...
            s% xtra(x_vsurf_at_remove_surface) = s% u(k) !max(s% u(k),2*sqrt(2d0*standard_cgrav*s% m(k)/(pow(10d0,4.0d0)*Rsun)))
