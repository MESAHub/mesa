...
def precursor_diagnostics(
    ...
    ahead = (radius > shock_radius + 5.0 * local_dr) & (
        radius < shock_radius + 0.1
    )
    if not np.any(ahead):
    ...
    behind = (radius < shock_radius - 5.0 * local_dr) & (
        radius > shock_radius - 0.2
    )
    exact_density = np.interp(
        radius[behind], exact["radius"], exact["density"]
    )
    density_postshock_error = (
        mesa["density"][behind] - exact_density
    ) / float(exact["rho_postshock"])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--time", type=float, default=0.36, help="target time in seconds")
...
def main() -> None:
    ...
    for row, (field, ylabel) in enumerate(fields):
        ...
        for run_number, (name, _, header, mesa, exact) in enumerate(selected):
            ...
            profile_axis.axvline(
                float(exact["shock_radius"]), color=color, alpha=0.25
            )

            exact_at_mesa = np.interp(
                mesa["radius"], exact["radius"], exact[field]
            )
            residual = (mesa[field] - exact_at_mesa) / float(exact[scales[field]])
            k_shock = int(
                np.argmin(abs(mesa["radius"] - float(exact["shock_radius"])))
            )
            k_lo = max(0, k_shock - 5)
            k_hi = min(mesa["radius"].size - 1, k_shock + 5)
            local_dr = np.median(np.diff(mesa["radius"][k_lo : k_hi + 1]))
            away_from_shock = (
                abs(mesa["radius"] - float(exact["shock_radius"]))
                > 5.0 * local_dr
            )
            residual_axis.plot(
    ...
    for run_number, (name, _, _, mesa, exact) in enumerate(selected):
        color = colors[run_number % len(colors)]
        exact_density = np.interp(
            mesa["radius"], exact["radius"], exact["density"]
        )
        density_residual = (
            mesa["density"] - exact_density
        ) / float(exact["rho_postshock"])
        mesh_axes[0].plot(
            mesa["radius"], density_residual, color=color, label=name
        )
    ...
    mesh_axes[1].axhline(
        args.max_long, color="black", linestyle=":", linewidth=1.0
    )
    mesh_axes[1].axhline(
