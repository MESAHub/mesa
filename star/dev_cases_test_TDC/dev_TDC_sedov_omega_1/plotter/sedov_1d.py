...
def sed_1d(time, nstep, xpos, eblast, rho0, vel0, ener0, pres0, cs0, gv):
    # ==============================================================================
    ...
    gv.a1 = (gv.xg2 * gv.gamma / (2.0e0 + gv.xgeom * gv.gamm1)) * (
        ((2.0e0 * (gv.xgeom * (2.0e0 - gv.gamma) - gv.omega)) / (gv.gamma * gv.xg2**2))
        - gv.a2
    )
    ...
    gv.d_val = (gv.xg2 * gv.gamp1) / (
        (gv.xg2 * gv.gamp1) - 2.0e0 * (2.0e0 + (gv.xgeom * gv.gamm1))
    )
    ...
    if gv.lsingular:
        ...
        alpha = (
            gv.gpogm
            * 2 ** (gv.xgeom)
            / (gv.xgeom * ((gv.gamm1 * gv.xgeom) + 2.0e0) ** 2)
        )
        if int(gv.xgeom) != 1:
            alpha = np.pi * alpha
