...
! ***********************************************************************
      ...
      subroutine do_merge(s, i_merge, species, new_xa, ierr)
         ...
         do i0 = ip+1, nz
            ...
            s% Dmix_explicit(im) = s% Dmix_explicit(i0)
            s% Dmix_implicit(im) = s% Dmix_implicit(i0)
            s% sig_implicit_ad(im) = s% sig_implicit_ad(i0)
            s% brunt_B_ad(im) = s% brunt_B_ad(i0)
            s% gradL_composition_term_ad(im) = s% gradL_composition_term_ad(i0)
      ...
      subroutine do_split(s, i_split, species, tau_center, grad_xa, new_xa, ierr)
         ...
         if (i_split < nz_old) then  ! move i_split..nz-1 to i_split+1..nz
            do j=nz-1,i_split,-1
               ...
               s% Dmix_explicit(jp) = s% Dmix_explicit(j)
               s% Dmix_implicit(jp) = s% Dmix_implicit(j)
               s% sig_implicit_ad(jp) = s% sig_implicit_ad(j)
               s% brunt_B_ad(jp) = s% brunt_B_ad(j)
               s% gradL_composition_term_ad(jp) = s% gradL_composition_term_ad(j)
   ...
   !$omp end critical  (adjust_mesh_split_merge_crit2)
         ...
         s% Dmix_explicit(ip) = s% Dmix_explicit(i)
         s% Dmix_implicit(ip) = s% Dmix_implicit(i)
         s% sig_implicit_ad(ip) = s% sig_implicit_ad(i)
         s% brunt_B_ad(ip) = s% brunt_B_ad(i)
         s% gradL_composition_term_ad(ip) = s% gradL_composition_term_ad(i)
