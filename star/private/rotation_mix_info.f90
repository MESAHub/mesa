...
! ***********************************************************************
      ...
      subroutine set_rotation_mixing_info(s, ierr)
         ...
         !real(dp), dimension(:), allocatable :: D_omega
         ...
         allocate( &
            csound(nz), rho(nz), T(nz), P(nz), cp(nz), cv(nz), chiRho(nz), &
            abar(nz), zbar(nz), gradT_sub_grada(nz), &
            opacity(nz), gamma1(nz), mu_alt(nz), eps_nuc(nz), enu(nz), L_neu(nz), delta(nz), &
            scale_height(nz), omega(nz), cell_dr(nz), &
            dRho(nz), dr(nz), dPressure(nz), domega(nz), d_mu(nz), d_j_rot(nz), &
            dRho_dr(nz), dRho_dr_ad(nz), dr2omega(nz), H_T(nz), &
            domega_dlnR(nz), Hj(nz), dlnR_domega(nz), &
            t_dyn(nz), t_kh(nz), Ri_mu(nz), Ri_T(nz), ve0(nz), ve_mu(nz), &
