...
!         GAMMAR => TDC_alfar*x_GAMMAR
      ...
      subroutine get1_tdc_L_eqn( &  
         ...
         ierr = 0
         L_expected = get_L_face(s, k, ierr)
         if (ierr /= 0) return        
...
!$omp end critical (hydro_equ_turbulent_crit1)
      ...
      function compute_Hp_face(s, k, ierr) result(Hp_face)
         type (star_info), pointer :: s
         integer, intent(in) :: k
         integer, intent(out) :: ierr
      ...
      function compute_Hp_cell(s, k, ierr) result(Hp_cell)
         type (star_info), pointer :: s
         integer, intent(in) :: k
         integer, intent(out) :: ierr
      ...
      function compute_Y_face(s, k, ierr) result(Y_face)
         type (star_info), pointer :: s
         integer, intent(in) :: k
         integer, intent(out) :: ierr
         type(auto_diff_real_18var_order1) :: Y_face
         type(auto_diff_real_18var_order1) :: Hp_face, Y1, Y2, &
         ...
         Y1 = 0.5d0*(QQ_00/Cp_00 + QQ_m1/Cp_m1)*(P_m1 - P_00) - (lnT_m1 - lnT_00)
      ...
      function compute_PII_face(s, k, ierr) result(PII_face)
         type (star_info), pointer :: s
         integer, intent(in) :: k
         type(auto_diff_real_18var_order1) :: PII_face
         integer, intent(out) :: ierr
         type(auto_diff_real_18var_order1) :: Cp_00, Cp_m1, Y_face
         real(dp) :: ALFAS, ALFA
         include 'formats'
         ierr = 0
         if (k == 1) then
         ...
         ALFAS = x_ALFAS
         ALFA = s% TDC_alfa
         PII_face = ALFAS*ALFA*0.5d0*(Cp_00 + Cp_m1)*Y_face
      end function compute_PII_face

      
      function compute_Chi_cell(s, k, ierr) result(Chi_cell) ! eddy viscosity (Kuhfuss 1986)
         type (star_info), pointer :: s
         integer, intent(in) :: k
         ...
         type(auto_diff_real_18var_order1) :: &
            POM1, POM2, d_v_div_r, &
            Hp_cell, v_00, v_p1, r_00, r_p1, w_00, d_00
         real(dp) :: POM, ALFAM, ALFA
         include 'formats'
         ierr = 0
         ALFAM = s% TDC_alfam
         ALFA = s% TDC_alfa
         ...
         w_00 = wrap_w_00(s,k)
         d_00 = wrap_d_00(s,k)
         POM = (16d0/3d0)*pi*ALFA*ALFAM/s% dm(k)  
         POM1 = w_00*pow2(d_00)
         POM2 = 0.5d0*(pow6(r_00) + pow6(r_p1))
         d_v_div_r = v_00/r_00 - v_p1/r_p1
         Chi_cell = POM*POM1*POM2*d_v_div_r*Hp_cell
      end function compute_Chi_cell

      
      function get_Eq_cell(s, k, ierr) result(Eq_cell) ! following RSP's definition
         type (star_info), pointer :: s
         integer, intent(in) :: k
         type(auto_diff_real_18var_order1) :: Eq_cell
         integer, intent(out) :: ierr
         type(auto_diff_real_18var_order1) :: &
            d_v_div_r, Chi_cell, v_00, v_p1, r_00, r_p1
         include 'formats'
         ierr = 0
         ...
         r_p1 = wrap_r_p1(s,k)
         d_v_div_r = v_00/r_00 - v_p1/r_p1
         Eq_cell = d_v_div_r*Chi_cell
      end function get_Eq_cell


      function get_Uq_face(s, k, ierr) result(Uq_face) ! following RSP's definition
         ! Inputs
         type (star_info), pointer :: s
         integer, intent(in) :: k
      ...
      function compute_C(s, k, ierr) result(C) ! following RSP's definition
         type (star_info), pointer :: s
         integer, intent(in) :: k
         type(auto_diff_real_18var_order1) :: C
         integer, intent(out) :: ierr
      ...
      function compute_Source(s, k, ierr) result(Source) ! following RSP's definition
         type (star_info), pointer :: s
         integer, intent(in) :: k
         ...
         type(auto_diff_real_18var_order1) :: &
            ...
            Hp_face_00, Hp_face_p1, PII_face_00, PII_face_p1, POM, POM2, POM3
         include 'formats'
         ierr = 0
         ...
         POM = 0.5d0*(PII_face_00/Hp_face_00 + PII_face_p1/Hp_face_p1)
         POM2 = T_00*P_00*QQ_00/Cp_00
         POM3 = w_00
         Source = POM*POM2*POM3            
      ...
      function compute_D(s, k, ierr) result(D) ! following RSP's definition
         type (star_info), pointer :: s
         integer, intent(in) :: k
      ...
      function compute_Dr(s, k, ierr) result(Dr) ! following RSP's definition
         type (star_info), pointer :: s
         integer, intent(in) :: k
         ...
         Hp_cell = compute_Hp_cell(s,k,ierr)
         if (ierr /= 0) return
         POM = 4d0*boltz_sigma*(gammar/alpha)**2
      ...
      function get_L_face(s, k, ierr) result(L_face)
         ! Inputs
         type (star_info), pointer :: s
         integer, intent(in) :: k
         ...
         call compute_L(s, k, L_face, Lr, Lc, Lt, ierr)
         return
      end function get_L_face


      function compute_Lr(s, k, ierr) result(Lr)
         type (star_info), pointer :: s
         integer, intent(in) :: k
      ...
      function compute_Lc(s, k, ierr) result(Lc)
         type (star_info), pointer :: s
         integer, intent(in) :: k
      ...
      subroutine reset_w_using_L(s, ierr)
         ...
         do k=1, nz
            ...
            s% xh(i_w,k) = w_00
            s% w(k) = w_00
            call compute_L(s, k, L, Lr, Lc, Lt, ierr) ! redo with new w(k)
