...
! ***********************************************************************
      ...
      use auto_diff_support, only: &
         ...
         wrap_chiT_00, wrap_d_00, wrap_d_m1, wrap_etrb_00, wrap_etrb_cell, &
         wrap_lnd_00, wrap_lnd_m1, rsp2_zero_w, &
      ...
      public :: rsp2_pressure_inertia_for_star_LNA
      ...
      subroutine rsp2_convective_luminosity_for_star_LNA(s, k, Lc_ad)
         ...
         Lc_ad = 0d0
         if (k <= 1 .or. k > s% nz .or. rsp2_zero_w(s,k)) return
      ...
      subroutine tdc_mixing_length_for_star_LNA(s, r_ad, Peos_ad, rho_ad, grav_ad, &
            scale_height_ad, Hp_for_mlt_ad, mixing_length_alpha_ad, Lambda_ad)
         type(star_info), pointer :: s
         type(auto_diff_real_star_order1), intent(in) :: &
            r_ad, Peos_ad, rho_ad, grav_ad, scale_height_ad
         type(auto_diff_real_star_order1), intent(out) :: &
            Hp_for_mlt_ad, mixing_length_alpha_ad, Lambda_ad

         if (s% harmonic_dissipation_length_beta > 0d0) then
            if (Peos_ad > 0d0 .and. grav_ad > 0d0 .and. rho_ad > 0d0) then
               Hp_for_mlt_ad = Peos_ad/(rho_ad*grav_ad)
               Lambda_ad = get_mlt_mixing_length( &
                  s, Hp_for_mlt_ad, r_ad, s% mixing_length_alpha)
               mixing_length_alpha_ad = Lambda_ad/Hp_for_mlt_ad
            else
               Hp_for_mlt_ad = 0d0
            ...
            Hp_for_mlt_ad = scale_height_ad
            mixing_length_alpha_ad = s% mixing_length_alpha
            Lambda_ad = get_mlt_mixing_length( &
               s, Hp_for_mlt_ad, r_ad, s% mixing_length_alpha)
         end if
      end subroutine tdc_mixing_length_for_star_LNA
      ...
      subroutine tdc_relation_for_star_LNA(s, k, luminosity_resid_ad, &
         ...
         call set_TDC_LNA( &
            mixing_length_alpha_ad, Lambda_ad, s% TDC_alpha_D, s% TDC_alpha_R, &
      ...
      subroutine rsp2_turbulent_energy_rhs_for_star_LNA(s, k, rhs_ad, ierr)
         ...
         integer, intent(out) :: ierr
         type(auto_diff_real_star_order1) :: coupling_ad, dLt_dm_ad

         call rsp2_coupling_for_star_LNA(s,k,coupling_ad,ierr)
         if (ierr /= 0) return
         call rsp2_dLt_dm_for_star_LNA(s,k,dLt_dm_ad,ierr)
         if (ierr /= 0) return
         ! Pressure work multiplies d(ln rho)/dt in the face energy inertia.
         rhs_ad = coupling_ad - dLt_dm_ad
      ...
      subroutine turbulent_energy_inertia_for_star_LNA(s, k, turbulent_inertia_ad, ierr)
         ...
         if (s% RSP2_flag) then
            turbulent_inertia_ad = wrap_etrb_cell(s,k)
      ...
      subroutine rsp2_pressure_inertia_for_star_LNA(s,k,inertia_ad)
         type(star_info), pointer :: s
         integer, intent(in) :: k
         ...
         inertia_ad = 0d0
         if (.not. s% star_LNA_perturb_turbulent_pressure .or. rsp2_zero_w(s,k)) return
         dm_face = 0.5d0*(s% dm(k-1)+s% dm(k))
         inertia_ad = -(s% RSP2_alfap/3d0)*pow2(s% w(k))/dm_face* &
            (s% dm(k-1)*(wrap_lnd_m1(s,k)-s% lnd(k-1)) + &
             s% dm(k)*(wrap_lnd_00(s,k)-s% lnd(k)))
      end subroutine rsp2_pressure_inertia_for_star_LNA
      ...
      subroutine rsp2_turbulent_pressure_work_dm_for_star_LNA(s,k,dwork_dm_ad,ierr)
         ...
         integer, intent(out) :: ierr
         type(auto_diff_real_star_order1) :: dVdt, dVdt_outer
         real(dp) :: dm_face
         ...
         if (.not. s% star_LNA_perturb_turbulent_pressure .or. rsp2_zero_w(s,k)) return
         dm_face = 0.5d0*(s% dm(k-1)+s% dm(k))
         call cell_dVdt_for_star_LNA(s,k,dVdt)
         call cell_dVdt_for_star_LNA(s,k-1,dVdt_outer)
         dwork_dm_ad = (s% RSP2_alfap/3d0)*pow2(s% w(k))/dm_face* &
            (s% rho(k)*dVdt + s% rho(k-1)*shift_m1(dVdt_outer))
      end subroutine rsp2_turbulent_pressure_work_dm_for_star_LNA


      subroutine rsp2_dLt_dm_for_star_LNA(s,k,dLt_dm_ad,ierr)
         type(star_info), pointer :: s
         integer, intent(in) :: k
         type(auto_diff_real_star_order1), intent(out) :: dLt_dm_ad
         integer, intent(out) :: ierr
         type(auto_diff_real_star_order1) :: Lt_outer, Lt_inner
         ...
         if (ierr /= 0) return
         dLt_dm_ad = (shift_m1(Lt_outer)-Lt_inner)/(0.5d0*(s% dm(k-1)+s% dm(k)))
      ...
      subroutine rsp2_turbulent_energy_inertia_for_star_LNA(s, k, inertia_ad, ierr)
         ...
         type(auto_diff_real_star_order1) :: pressure_inertia

         ierr = 0
         call rsp2_pressure_inertia_for_star_LNA(s,k,pressure_inertia)
         inertia_ad = wrap_etrb_00(s,k) + pressure_inertia
      ...
      subroutine Uq_cell_for_star_LNA(s, k, Uq_ad, ierr)
         type(star_info), pointer :: s
         integer, intent(in) :: k
         type(auto_diff_real_star_order1), intent(out) :: Uq_ad
         integer, intent(out) :: ierr
         real(dp) :: r_cell
         type(auto_diff_real_star_order1) :: Chi_00, Chi_p1
         ...
         r_cell = s% rmid(k)
         if (r_cell <= 0d0) then
            write(*,'(a,i0)') 'star_LNA found nonpositive cell radius for u momentum at k = ', k
            ierr = -1
            return
         end if
         Uq_ad = 4d0*pi*(Chi_00 - Chi_p1)/(r_cell*s% dm(k))
      end subroutine Uq_cell_for_star_LNA
      ...
      subroutine static_eddy_Chi_face_for_star_LNA(s, k, Chi_ad, ierr)
         ...
         real(dp) :: alpha_M, num_outer, num_inner
      ...
      real(dp) function chi_geometry_coefficient_for_star_LNA( &
            s, k, alfam, length_cell, w_cell) result(chi_coeff)
         type(star_info), pointer :: s
         integer, intent(in) :: k
         real(dp), intent(in) :: alfam, length_cell, w_cell
         real(dp) :: r00, rp1, r6_cell

         if (w_cell <= 0d0 .or. length_cell <= 0d0) then
            chi_coeff = 0d0
            return
         end if
