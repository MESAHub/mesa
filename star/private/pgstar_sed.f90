...
contains
   ...
   subroutine do_SED_Plot1(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      ...
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED1_lambda_min, s% pg% SED1_lambda_max, s% pg% SED1_flux_min, s% pg% SED1_flux_max, &
         s% pg% SED1_filter_scale, ierr)
   end subroutine do_SED_Plot1

   subroutine do_SED_Plot2(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED2_lambda_min, s% pg% SED2_lambda_max, s% pg% SED2_flux_min, s% pg% SED2_flux_max, &
         s% pg% SED2_filter_scale, ierr)
   end subroutine do_SED_Plot2

   subroutine do_SED_Plot3(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED3_lambda_min, s% pg% SED3_lambda_max, s% pg% SED3_flux_min, s% pg% SED3_flux_max, &
         s% pg% SED3_filter_scale, ierr)
   end subroutine do_SED_Plot3

   subroutine do_SED_Plot4(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED4_lambda_min, s% pg% SED4_lambda_max, s% pg% SED4_flux_min, s% pg% SED4_flux_max, &
         s% pg% SED4_filter_scale, ierr)
   end subroutine do_SED_Plot4

   subroutine do_SED_Plot5(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED5_lambda_min, s% pg% SED5_lambda_max, s% pg% SED5_flux_min, s% pg% SED5_flux_max, &
         s% pg% SED5_filter_scale, ierr)
   end subroutine do_SED_Plot5

   subroutine do_SED_Plot6(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED6_lambda_min, s% pg% SED6_lambda_max, s% pg% SED6_flux_min, s% pg% SED6_flux_max, &
         s% pg% SED6_filter_scale, ierr)
   end subroutine do_SED_Plot6

   subroutine do_SED_Plot7(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED7_lambda_min, s% pg% SED7_lambda_max, s% pg% SED7_flux_min, s% pg% SED7_flux_max, &
         s% pg% SED7_filter_scale, ierr)
   end subroutine do_SED_Plot7

   subroutine do_SED_Plot8(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED8_lambda_min, s% pg% SED8_lambda_max, s% pg% SED8_flux_min, s% pg% SED8_flux_max, &
         s% pg% SED8_filter_scale, ierr)
   end subroutine do_SED_Plot8

   subroutine do_SED_Plot9(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, ierr)
      type(star_info), pointer :: s; integer :: id, device_id, ierr; real :: xleft, xright, ybot, ytop, txt_scale; logical :: subplot; character(len=*) :: title
      call render_sed_core(s, id, device_id, xleft, xright, ybot, ytop, subplot, title, txt_scale, &
         s% pg% SED9_lambda_min, s% pg% SED9_lambda_max, s% pg% SED9_flux_min, s% pg% SED9_flux_max, &
         s% pg% SED9_filter_scale, ierr)
   ...
   subroutine render_sed_core(s, id, device_id, vp_xleft, vp_xright, vp_ybot, vp_ytop, subplot, title, txt_scale, &
                              w_min, w_max, f_min, f_max, filter_scale, ierr)
      ...
      allocate(x_wave(n_lam), y_flux(n_lam), window_mask(n_lam), stat=ierr)
      if (ierr /= 0) return
      ...
      wave_min = merge(minval(x_wave), w_min, w_min == -101.0e0)
      wave_max = merge(maxval(x_wave), w_max, w_max == -101.0e0)

      window_mask = (x_wave >= wave_min) .and. (x_wave <= wave_max) .and. (y_flux > -49.0e0)

      if (f_max == -101.0e0) then
         if (any(window_mask)) then
            flux_max = maxval(y_flux, mask=window_mask) + 0.1e0
         else
            flux_max = maxval(y_flux, mask=(y_flux > -49.0e0)) + 0.1e0
         end if
      else
         flux_max = f_max
      end if

      if (f_min == -101.0e0) then
         if (any(window_mask)) then
            flux_min = minval(y_flux, mask=window_mask) - 0.1e0
         else
            flux_min = flux_max - 4.0e0
         end if
      else
         flux_min = f_min
      end if
      ...
      if (cs% filters_loaded .and. filter_scale > 0.0e0) then
         do i = 1, size(cs% filters)
            ...
            ! Dynamic allocation based directly on user-defined filter_scale factor (0.0 to 1.0)
            filter_y = flux_max - ((1.0e0 - real(cs% filters(i)% transmission)) * (flux_max - flux_min) * filter_scale)
            ...
            call pgptxt(filter_x(k/2), flux_max - (0.04e0 * filter_scale) * (flux_max - flux_min), 0.0e0, 0.5e0, trim(cs% filters(i)% name))
            call pgsch(txt_scale)
      ...
      call pgsls(1) 
      call pgsci(clr_Foreground)
      deallocate(x_wave, y_flux, window_mask)
