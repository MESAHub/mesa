...
!      end subroutine RSP_mesh
   ...
   subroutine RSP_mesh( &
      ...
      integer :: nz, k
      real(dp), pointer :: vals(:, :)
      real(dp), parameter :: weight1 = 5d3 !1d4
      real(dp), parameter :: weight2 = 1d2 !1d4
...
!         end do
      ...
      do k = 1, nz
         ...
         if (s%lnT(k) < logT_anchor1) then
            ...
            ! envelope weighting
            vals(k, 1) = 0d0!weight2*(s%m(k_anchor1-1) - s%m(k))/Msun
            k_anchor2 = k
         else
            ! smooth taper from weight2 → 0
            vals(k, 1) = 0d0!(
            !0.5d0*weight2*(1d0 - tanh( (ell - lmid)/delta )) &
            !) * ( (s%m(k_anchor2-1) - s%m(k)) / Msun )
         end if
      end do
