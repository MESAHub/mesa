...
phase_colors = np.asarray(checker.phase_colors, object) if len(checker.phase_colors) else None
...
with writer.saving(fig, OUT, dpi=DPI):
    ...
    for i in iterator:
        ...
        C = phase_colors[sl] if phase_colors is not None and len(phase_colors) == N else None

        # === Top-left: CMD/HR (color vs mag) ===
        axes[0, 0].plot(hr_x[sl], hr_y[sl], marker=",", linestyle="-", color="k", alpha=0.2)
        if C is not None:
            axes[0, 0].scatter(hr_x[sl], hr_y[sl], c=C, s=15, alpha=0.9, edgecolors="none")
        else:
        ...
        axes[0, 1].plot(Teff[sl], Log_L[sl], marker=",", linestyle="-", color="k", alpha=0.2)
        if C is not None:
            axes[0, 1].scatter(Teff[sl], Log_L[sl], c=C, s=15, alpha=0.9, edgecolors="none")
        else:
        ...
        axes[1, 0].plot(age[sl], checker.color_index[sl], marker=",", linestyle="-", color="k", alpha=0.2)
        if C is not None:
            axes[1, 0].scatter(age[sl], checker.color_index[sl], c=C, s=15, alpha=1.0, edgecolors="none")
        else:
            axes[1, 0].scatter(age[sl], checker.color_index[sl], s=15, alpha=1.0, edgecolors="none")
        ...
        for label, series, color in mag_series:
            axes[1, 1].plot(age[sl], series[sl], marker="o", linestyle="-", markersize=3, alpha=0.8, label=label, color=color)
