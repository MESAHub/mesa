...
def print_summary_table(results: list[dict]):
    ...
    for r in results:
        ...
        if (
            r["s_per_step"] is not None
            and baseline_sps is not None
            and baseline_sps > 0
        ):
...
def plot_wall_time(results: list[dict], out: Path):
    ...
    bars = ax.bar(
        labels, walls, color=colors, edgecolor="white", linewidth=0.8, zorder=3
    )
...
def plot_overhead(results: list[dict], out: Path):
    ...
    overhead = [
        100.0 * (r["s_per_step"] - baseline_sps) / baseline_sps for r in non_baseline
    ]
    colors = [PALETTE[results.index(r)] for r in non_baseline]

    fig, ax = plt.subplots(figsize=(10, 5))
    bars = ax.bar(
        labels, overhead, color=colors, edgecolor="white", linewidth=0.8, zorder=3
    )
