...
class SEDChecker:
    ...
    def __init__(self, directory=None, xlim=None, ylim=None, refresh_interval=0.1,
                 save_video=False, video_filename="sed_animation.mp4", video_fps=10, 
                 video_duration=30, inlist_file="../inlist_colors"):
        """
        ...
        try:
            ...
            for line in lines:
                ...
                if 'colors_results_directory' in line:
                    ...
                    value = line.split('=')[1].strip().replace('d', 'e')  # fix fortran notation
                    try:
    ...
    def setup_plot(self):
        ...
        if stellar_info:
            self.ax.text(0.99, 0.98, stellar_info, transform=self.ax.transAxes,
    ...
    def add_em_spectrum_regions(self):
        ...
        for min_wave, max_wave, label, color in em_regions:
            ...
            center_wave = np.sqrt(visible_min * visible_max)  # geometric mean for log scale
            self.ax.text(center_wave, 0.95, label, 
    ...
    def update_plot(self, frame):
        ...
        for file_path in output_files:
            ...
            if "VEGA" not in file_path and "bright" not in file_path and "faint" not in file_path:
                ...
                try:
                    ...
                    if len(wavelengths) > 0 and len(flux) > 0 and not self.full_sed_plotted:
                        line = self.ax.plot(
                            wavelengths, flux, color="black", linewidth=1.5, linestyle=linestyle,
                        )[0]
                    ...
                    if len(wavelengths) > 0 and len(convolved_flux) > 0:
                        ...
                        if short_label and short_label not in displayed_labels:
                            line = self.ax.plot(
                                wavelengths, convolved_flux, linewidth=1,
        ...
        if legend_handles and legend_labels_list:
            self.ax.legend(legend_handles, legend_labels_list, loc="lower right", fontsize=10)
        ...
        if not self.xlim and sed_wavelengths and sed_flux_data:
            ...
            if max_sed_flux > 0:
                ...
                if len(active_wavelengths) > 0:
                    ...
                    self.ax.set_xlim([min_wave - padding * 0.01, max_wave + padding * 10])
        ...
        if self.save_video:
            ...
            try:
                ...
                self.animation = FuncAnimation(
                    self.fig, self.update_plot, frames=self.video_frame_count,
                    interval=self.refresh_interval * 1000, cache_frame_data=False,
                )
