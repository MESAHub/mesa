...
def load_history_data(
    ...
    try:
        ...
        print_success(
            f"Loaded history file: {history_file} ({len(md.model_number)} models)"
        )
        return md
...
def parse_expression(
    expr: str, column_names: List[str], data: np.ndarray
    ...
    def replace_index(match):
        ...
        if idx < 0 or idx >= len(column_names):
            raise ValueError(
                f"Column index [{idx}] out of range (0-{len(column_names) - 1})"
            )
...
def resolve_axis(
    spec: Union[int, str], column_names: List[str], data: np.ndarray
) -> Tuple[np.ndarray, str]:
    """
...
def create_plot(
    ...
    if z_data is not None:
        ...
        scatter = ax.scatter(
            x_data, y_data, z_data, c=color_data, cmap=cmap, s=point_size, alpha=alpha
        )
        ...
        scatter = ax.scatter(
            x_data, y_data, c=color_data, cmap=cmap, s=point_size, alpha=alpha
        )
...
def save_figure(
    fig: plt.Figure, base_name: str, formats: List[str], dpi: int = 300
) -> None:
...
Supported expression syntax:
  ...
  - Functions: sqrt(), log10(), abs()
    ...
    parser.add_argument(
        "-x", "--x-col", help="X-axis column name or index (enables batch mode)"
    )

    parser.add_argument("-y", "--y-col", help="Y-axis column name or index")

    parser.add_argument(
        "-z", "--z-col", help="Z-axis column name or index (for 3D plots)"
    )

    parser.add_argument("-c", "--color-col", help="Color axis column name or index")

    parser.add_argument(
        "-o", "--output", help="Output filename base (without extension)"
    )
    ...
    parser.add_argument(
        "--no-show", action="store_true", help="Don't display interactive plot"
    )

    parser.add_argument(
        "--list-columns", action="store_true", help="List available columns and exit"
    )
