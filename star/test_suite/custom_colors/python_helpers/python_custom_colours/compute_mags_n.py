...
def compute_filter_magnitude(
    ...
    print(
        "Scaled Flux Lambda (middle 5):",
        flux_lambda[int(len(flux_lambda) / 2) : int(len(flux_lambda) / 2) + 5],
    )

    # Clip stellar spectrum to filter wavelength range
    mask = (wavelength_angstrom >= wavelength_min) & (
        wavelength_angstrom <= wavelength_max
    )
    wavelength_angstrom_clipped = wavelength_angstrom[mask]
    flux_lambda_clipped = flux_lambda[mask]
    print(
        "Clipped Flux Lambda (middle 5):",
        flux_lambda_clipped[
            int(len(flux_lambda_clipped) / 2) : int(len(flux_lambda_clipped) / 2) + 5
        ],
    )
    ...
    print(
        "Filter Response Clipped (middle 5):",
        filter_response_clipped[
            int(len(filter_response_clipped) / 2) : int(
                len(filter_response_clipped) / 2
            )
            + 5
        ],
    )
    ...
    integrand = (
        observed_flux_lambda * wavelength_angstrom_clipped * 1e-8
    )  # Convert Å to cm
