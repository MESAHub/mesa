...
wavelength_nm = np.linspace(
    min(filter_wavelength_nm), max(filter_wavelength_nm), len(filter_wavelength_nm)
)  # in nanometers
...
filter_interp = interp1d(
    filter_wavelength_nm, filter_transmission, bounds_error=False, fill_value=0
)
...
plt.plot(
    ...
    filter_transmission * max(B_lambda),
