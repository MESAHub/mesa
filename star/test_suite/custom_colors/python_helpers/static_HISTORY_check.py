...
def create_phase_plots(history_file="../LOGS/history.data"):
    ...
    hr_color, hr_mag, hr_xlabel, hr_ylabel, color_index = setup_hr_diagram_params(md, filter_columns)
    ...
    # Create the plots
    fig, axes = plt.subplots(2, 2, figsize=(14, 18), gridspec_kw={"hspace": 0.3, "wspace": 0.2})
    
    # Top-left plot: HR Diagram (Color vs. Magnitude) with phase colors
    scatter1 = axes[0, 0].scatter(hr_color, hr_mag, c=phase_colors, s=20, alpha=0.7, edgecolors='none')
    ...
    scatter2 = axes[0, 1].scatter(Teff, Log_L, c=phase_colors, s=20, alpha=0.7, edgecolors='none')
    ...
    scatter3 = axes[1, 0].scatter(Star_Age, color_index, c=phase_colors, s=20, alpha=0.7, edgecolors='none')
    ...
    for filt in filter_columns:
        ...
        axes[1, 1].plot(Star_Age, col_data, marker="o", linestyle="-", label=filt, markersize=3, alpha=0.8)
    ...
    legend_elements = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, 
                                 markersize=8, label=phase, markeredgecolor='none') 
