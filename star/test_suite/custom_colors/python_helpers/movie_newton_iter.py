...
def sigma_clip_mask(
    data_arrays: List[np.ndarray], sigma: float = 3.0, max_iter: int = 5
) -> np.ndarray:
    """
...
def sort_by_model_and_iter(
    data: np.ndarray, column_names: List[str]
...
def create_video(
    ...
    if n_removed > 0:
        print_info(
            f"Removed {n_removed} outlier points ({100 * n_removed / len(x_data):.1f}%)"
        )
    ...
    has_history = (
        history_x is not None and history_y is not None and len(history_x) >= n_models
    )

    if has_history:
        print_info(
            f"History data: {len(history_x)} points available, {n_models} will be shown incrementally"
        )
    ...
    def update(frame):
        ...
        if has_history:
            ...
            if n_history_show > 0:
                scatter_history.set_offsets(
                    np.column_stack(
                        [history_x[:n_history_show], history_y[:n_history_show]]
                    )
                )
            else:
                scatter_history.set_offsets(np.empty((0, 2)))
        ...
        title.set_text(
            f"Newton Iteration Colors — Model {current_model}, Iter {current_iter}\n"
    ...
    if output_path.endswith(".gif"):
        ...
        try:
            writer = FFMpegWriter(
                fps=fps, metadata={"title": "Newton Iterations"}, bitrate=5000
            )
        except Exception:
...
Examples:
  ...
  %(prog)s -x Teff -y V -c model --sigma 2.5  # Stricter outlier removal
    ...
    parser.add_argument(
        "-c", "--color-col", help="Color axis column name or expression"
    )
    ...
    parser.add_argument(
        "--list-columns", action="store_true", help="List columns and exit"
    )
