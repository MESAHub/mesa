...
! ***********************************************************************
      ...
      ! Record the central Gamma1 at the first pair-instability onset.
      integer, parameter :: lx_have_gamma1_cntr_pulse_start = 6
      ...
      integer, parameter :: x_gamma1_cntr_pulse_start = 9
      ...
      subroutine extras_startup(id, restart, ierr)
         ...
         if (.not. restart) then
            ...
            s% lxtra(lx_have_gamma1_cntr_pulse_start) = .false.
            s% xtra(x_gamma1_cntr_pulse_start) = 0d0
            s% lxtra(lx_using_direct_removal_bcs) = .false.
      ...
      subroutine extras_after_evolve(id, ierr)
         ...
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         if (in_inlist_pulses .and. s% lxtra(lx_have_gamma1_cntr_pulse_start)) then
            testhub_extras_names(1) = 'gamma1_cntr_pulse_start'
            testhub_extras_vals(1) = s% xtra(x_gamma1_cntr_pulse_start)
         end if
      ...
      integer function extras_start_step(id)
         ...
         if (s% u_flag .and. k_keep > 0 .and. s% xtra(x_time_start_pulse) > 0d0) then
            ...
            if (s% q(k_keep) > 1d-3) then
               ...
               if (k0 == 0) then
                  extras_start_step = terminate
                  if (.not. s% lxtra(lx_have_gamma1_cntr_pulse_start)) then
                     write(*,'(a)') 'PISN disruption without a recorded pair-instability onset'
                     return
                  end if
                  ...
                  termination_code_str(t_xtra2) = 'Successful test: PISN disruption'
         ...
         gamma1_integral = gamma1_integral/max(1d-99,integral_norm)
         if (in_inlist_pulses .and. gamma1_integral < 0d0 .and. &
               .not. s% lxtra(lx_have_gamma1_cntr_pulse_start)) then
            s% xtra(x_gamma1_cntr_pulse_start) = s% gamma1(s% nz) - 4d0/3d0
            s% lxtra(lx_have_gamma1_cntr_pulse_start) = .true.
         end if
