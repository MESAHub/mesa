...
for i in range(1, imax + 1, 1):
    ...
    my.print_parameters(
        mp, rp, mcore, rhocore, mp_wo_core, irrad_col, flux_dayside, Teq, y, z, maxage
    )
    ...
    if do_put_in_core:
        if mcore > 0.0:
            ...
            run_time = my.put_core_in_planet(
                mcore, rhocore, inlist2, createmodel, coremodel
            )
        else:
    ...
    if do_evolve_planet:
        ...
        run_time = my.evolve_planet(
            irrad_col, flux_dayside, maxage, inlist3, coremodel, evolvemodel
        )
