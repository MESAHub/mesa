program test_memory
  ...
  do i = 1, 2
     ...
     write(*,*) 'id =', id
     if (ierr /= 0) stop 'error in alloc_star'
     call star_ptr(id, s, ierr)
     if (ierr /= 0) stop 'error in star_ptr'
     s% inlist_fname = 'inlist'
     call read_star_job(s, '', ierr)
     if (ierr /= 0) stop 'error in read_star_job'
     call starlib_init(s, ierr)
     if (ierr /= 0) stop 'error in starlib_init'
     call star_set_kap_and_eos_handles(id, ierr)
     if (ierr /= 0) stop 'error in star_set_kap_and_eos_handles'
     call star_setup(id, '', ierr)
     if (ierr /= 0) stop 'error in star_setup'
     ...
     if (ierr /= 0) stop 'error in star_create_pre_ms_model'
     call starlib_shutdown()
     call free_star(id, ierr)
     if (ierr /= 0) stop 'error in free_star'
  end do
