...
contains
   ...
   subroutine do_newton_wrapper(nz, nvar, x, xold, matrix_type, mljac, mujac, &
                                decsol, decsolblk, lrd, rpar_decsol, lid, ipar_decsol, which_decsol, &
                                tol_correction_norm, set_primaries, set_secondaries, set_xscale, &
                                Bdomain, xdomain, eval_equations, size_equ, sizeb, inspectB, &
                                enter_setmatrix, exit_setmatrix, failed_in_setmatrix, force_another_iteration, &
                                xscale, equ, ldy, nsec, y, work, lwork, iwork, liwork, &
                                AF, lrpar, rpar, lipar, ipar, convergence_failure, ierr)
...
#include "newton_procs.dek"
   ...
   subroutine do_newton(nz, nvar, x1, xold1, AF1, ldAF, neq, matrix_type, mljac, mujac, &
                        decsol, decsolblk, lrd, rpar_decsol, lid, ipar_decsol, &
                        tol_correction_norm, set_primaries, set_secondaries, set_xscale, &
                        Bdomain, xdomain, eval_equations, size_equ, sizeb, inspectB, &
                        enter_setmatrix, exit_setmatrix, failed_in_setmatrix, force_another_iteration, &
                        xscale1, equ1, ldy, nsec, y_in1, work, lwork, iwork, liwork, &
                        lrpar, rpar, lipar, ipar, convergence_failure, ierr)
...
#include "newton_procs.dek"
      ...
      ! locals
      real(dp)  :: coeff, f, slope, residual_norm, max_residual, &
      ...
      do while (.not. passed_tol_tests)
         ...
         overlay_AF = (min_corr_coeff == 1) .and. &
                      (matrix_type == banded_matrix_type .or. matrix_type == block_tridiag_dble_matrix_type)
         ...
         call adjust_correction(min_corr_coeff, correction_factor, grad_f1, f, slope, coeff, err_msg, &
         ...
         if (max_correction > tol_max_correction*coeff .or. max_residual > tol_max_residual*coeff) then
            ...
            passed_tol_tests = (correction_norm <= tol_correction_norm*coeff .and. &
                                residual_norm <= tol_residual_norm*coeff) .or. &
         ...
         if (.not. passed_tol_tests) then
            if (iiter >= max_tries) then
               ...
               if (coeff < min(min_corr_coeff, correction_factor)) then
                  ...
                  exit
               else if (correction_norm > tol_correction_norm*coeff .and. &
                        (correction_norm > work(r_corr_norm_jump_limit)*corr_norm_min) &
                        .and. (.not. first_try)) then
                  call oops('avg correction jumped')
                  exit
               else if (residual_norm > tol_residual_norm*coeff .and. &
                        (residual_norm > work(r_resid_norm_jump_limit)*resid_norm_min) &
                        .and. (.not. first_try)) then
                  call oops('avg residual jumped')
                  exit
               else if (max_correction > tol_max_correction*coeff .and. &
                        (max_correction > work(r_max_corr_jump_limit)*max_corr_min) &
                        .and. (.not. first_try)) then
                  call oops('max correction jumped')
                  exit
               else if (residual_norm > tol_residual_norm*coeff .and. &
                        (max_residual > work(r_max_resid_jump_limit)*max_resid_min) &
                        .and. (.not. first_try)) then
   ...
   contains
      ...
      subroutine adjust_correction(min_corr_coeff_in, max_corr_coeff, grad_f, f, slope, coeff, err_msg, &
                                   lrpar, rpar, lipar, ipar, ierr)
      ...
      subroutine factor_mtx(n, ldafb, sprs_nz)
         ...
         if (matrix_type == block_tridiag_dble_matrix_type) then
            ...
            if (info /= 0) then
               call decsolblk(2, caller_id, nvar, nz, lblkF1, dblkF1, ublkF1, &
                              B1, ipiv_blk1, lrd, rpar_decsol, lid, ipar_decsol, info_dealloc)
            end if
         else if (matrix_type == square_matrix_type) then
            do k = 1, n*n
               AF1(k) = A1(k)
      ...
      subroutine solve_mtx(n, ldafb, sprs_nz)
         ...
         trans = 'N'
         if (matrix_type == block_tridiag_dble_matrix_type) then
            call decsolblk(1, caller_id, nvar, nz, lblkF1, dblkF1, ublkF1, &
                           B1, ipiv_blk1, lrd, rpar_decsol, lid, ipar_decsol, info_solve)
            call decsolblk(2, caller_id, nvar, nz, lblkF1, dblkF1, ublkF1, &
                           B1, ipiv_blk1, lrd, rpar_decsol, lid, ipar_decsol, info_dealloc)
         else if (matrix_type == square_matrix_type) then
      ...
      logical function do_enter_setmatrix(neq, x, dx, xscale, lrpar, rpar, lipar, ipar, ierr)
         ...
         call enter_setmatrix(iiter, nvar, nz, neq, x, xold, xscale, xder, need_solver_to_eval_jacobian, &
            ...
            do kk = 0, nskip(ivar) - 1
               ...
               do j = ivar + kk*nvar, neq, nvar*nskip(ivar)
                  ...
                  if (dxdd(ivar, zone) == dxsave(ivar, zone)) then
                     ...
                     ierr = -1
                     write (*, '(a, i5, 99e20.10)') 'failed trying to create numerical derivative for variable ', &
...
1        format(i6, 2x, i3, 2x, a, f8.4, 6(2x, a, 1x, e10.3), 2x, a, f6.2, 2x, a)
         write (*, 1) iwork(i_model_number), iiter, 'coeff', coeff, 'slope', slope, 'f', f, &
            'avg resid', residual_norm, 'max resid', max_residual, 'avg corr', correction_norm, 'max corr', max_correction, &
            'lg dt/yr', log10(max(1d-99, work(r_dt)/secyer)), trim(msg)
...
5        format(a20, i6, 1x, i16) ! 1 index integers
      ...
      subroutine pointers(ierr)
         ...
         if (i - 1 > lwork) then
            ierr = -1
            write (*, '(a, i6, a, 99i6)') 'newton: lwork is too small.  must be at least', i - 1, &
         ...
         if (i - 1 > liwork) then
            ierr = -1
            write (*, '(a, i6, a, i6)') 'newton: liwork is too small.  must be at least', i, &
               '   but is only ', liwork
            return
         end if
