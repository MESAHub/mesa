...
! ***********************************************************************
      ...
      real(dp) function do_safe_root_with_brackets(f,x1_in,x3_in,y1_in,y3_in,imax,epsx,epsy,lrpar,rpar,lipar,ipar,ierr)
...
#include "num_root_fcn.dek"
         ...
         do_safe_root_with_brackets = eval_brent_safe_zero(x1_in, x3_in, 1d-14, epsx, epsy, f, y1_in, y3_in,lrpar, rpar, lipar, ipar, ierr )

      end function do_safe_root_with_brackets


      real(dp) function do_safe_root_with_guess(f, x_guess, dx, x1_in, x3_in, y1_in, y3_in, newt_imax, imax,epsx, epsy, lrpar, rpar, lipar, ipar, ierr)
         interface
...
#include "num_root_fcn.dek"
         ...
         if (.not. (have_x1 .and. have_x3)) then
            call do_look_for_brackets(x_guess,dx,x1,x3,f,y1,y3,imax,lrpar,rpar,lipar,ipar,ierr)
            if (ierr /= 0) then
               if (dbg) then
         ...
         do_safe_root_with_guess = do_safe_root_with_brackets(f,x1,x3,y1,y3,imax,epsx,epsy,lrpar,rpar,lipar,ipar,ierr)
         if (ierr /= 0) then
            if (dbg) then
...
#include "num_root_fcn.dek"
      ...
      real(dp) function do_safe_root(f,x_guess,x1_in,x3_in,y1_in,y3_in,imax,epsx,epsy,lrpar,rpar,lipar,ipar,ierr)
         ...
         do_safe_root = eval_brent_safe_zero (x1_in, x3_in, 1d-14, epsx, epsy, f, y1_in, y3_in,lrpar, rpar, lipar, ipar, ierr )
