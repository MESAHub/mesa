! ***********************************************************************
!
!   Copyright (C) 2010  Bill Paxton, Frank Timmes & The MESA Team
!
...
contains
   ...
   subroutine read_data_for_scvh(data_dir)
      ...
      call read_file_for_scvh(data_dir, 'scvh/h_tab.asc.data', &
      ...
      if (.false.) then
         call write_file_for_scvh(data_dir, 'scvh/h_tab.new.data', &
                                  num_h_pts, NlogPs, NlogTs, compv1_h_si, compv2_h_si, denlog_h_si, slog_h_si, ulog_h_si)
         call write_file_for_scvh(data_dir, 'scvh/he_tab.new.data', &
                                  num_he_pts, NlogPs, NlogTs, compv1_he_si, compv2_he_si, denlog_he_si, slog_he_si, ulog_he_si)
   ...
   subroutine interp_densities(logT, logP, pres, den_h, den_he, info)
      ...
      double precision ::ener_h, entr_h, dddt_cp_h, dddpress_ct_h, &
         dsdt_cp_h, dsdpress_ct_h, ener_he, entr_he, dddt_cp_he, dddpress_ct_he, &
         dsdt_cp_he, dsdpress_ct_he, xnh, &
         dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, dxnhe_dlogP, xnhep, dxnhep_dlogT, dxnhep_dlogP
      logical, parameter :: only_densities = .true., search_for_SCVH = .false.
      call interp_vals_bicub(only_densities, search_for_SCVH, den_h, ener_h, entr_h, &
                             dddt_cp_h, dddpress_ct_h, dsdt_cp_h, dsdpress_ct_h, &
                             den_he, ener_he, entr_he, dddt_cp_he, dddpress_ct_he, &
                             dsdt_cp_he, dsdpress_ct_he, xnh, dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, &
                             xnhe, dxnhe_dlogT, dxnhe_dlogP, xnhep, dxnhep_dlogT, dxnhep_dlogP, logT, logP, pres, info)
   end subroutine interp_densities

   subroutine interp_vals_bicub(only_densities, search_for_SCVH, den_h, ener_h, entr_h, &
                                dddt_cp_h, dddpress_ct_h, dsdt_cp_h, dsdpress_ct_h, den_he, ener_he, entr_he, &
                                dddt_cp_he, dddpress_ct_he, dsdt_cp_he, dsdpress_ct_he, xnh, &
                                dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, &
                                xnhe, dxnhe_dlogT, dxnhe_dlogP, xnhep, dxnhep_dlogT, dxnhep_dlogP, logT, logP, pres, info)
      logical, intent(in) :: only_densities, search_for_SCVH
      double precision, intent(out) ::den_h, ener_h, entr_h, &
         dddt_cp_h, dddpress_ct_h, dsdt_cp_h, dsdpress_ct_h, den_he, ener_he, entr_he, &
         dddt_cp_he, dddpress_ct_he, dsdt_cp_he, dsdpress_ct_he, xnh, &
         dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, dxnhe_dlogP, xnhep, dxnhep_dlogT, dxnhep_dlogP
      double precision, intent(in) :: logT, logP, pres
   ...
   contains
      ...
      double precision function interp_value(n, si)
         ...
         if (si%needs_initialization) then
            ...
            ibcymin = 0; bcymin(:) = 0
            ibcymax = 0; bcymax(:) = 0
            call interp_mkbicub_db(logPs, NlogPs, logTs, NlogTs, si%f1, NlogPs, &
                                   ibcxmin, bcxmin, ibcxmax, bcxmax, &
                                   ibcymin, bcymin, ibcymax, bcymax, si%ilinx, si%iliny, ierr)
            if (ierr /= 0) then
               return
   ...
   subroutine interpolate_scvh(include_radiation, search_for_SCVH, logT, logRho, T, Rho, xmassh1, &
                               logPgas, logE, logS, chiRho, chiT, &
                               Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, gamma1, gamma3, grad_ad, logNe, info)
...
!..this routine interpolates the scvh tables
      ...
      double precision, intent(out) :: logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, &
         mu, gamma1, gamma3, grad_ad, logNe
      integer, intent(out) :: info ! returned = 0 if AOK

      double precision :: dpressdd, dpressdt, ener, dedd, xnh, dxnh_dlogT, dxnh_dlogP, xnh2, &
         dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, dxnhe_dlogP, xnhep, dxnhep_dlogT, dxnhep_dlogP, &
         xnhp, xnhepp, P, entr, dsdd, dsdt, xtra, dxdd, dxdt, Ne, log_free_e, log_free_e0, log_free_e1, kt, theta
...
!..for hydrogen
      double precision :: den_h, ener_h, entr_h, dddt_cp_h, dddpress_ct_h, &
         dsdt_cp_h, dsdpress_ct_h, dpressdd_h, dpressdt_h, dedd_h, dedt_h, dsdd_h, dsdt_h

!..for helium
      double precision :: den_he, ener_he, entr_he, dddt_cp_he, dddpress_ct_he, &
         dsdt_cp_he, dsdpress_ct_he, dpressdd_he, dpressdt_he, dedd_he, dedt_he, dsdd_he, dsdt_he

!..for the mixture
      double precision :: beta, gama, delt, smix, d_smix_dT, d_smix_dP, dddt_cp_hhe, dddpress_ct_hhe, &
         dsdt_cp_hhe, dsdpress_ct_hhe, dtdpress_cs_hhe, dpressdd_hhe, dpressdt_hhe, dedd_hhe, dedt_hhe, dsdd_hhe, dsdt_hhe
...
!..constants
      ...
      double precision :: dt, dt2, dti, dt2i, dpress, dpress2, dpressi, dpress2i, &
         xt, xp, mxt, mxp, logRho_new, dlogRho_dlogP, del_logP, under, &
...
!..find pressure that gives the desired density
      ...
      epslogP = 1d-14
      epslogRho = 1d-10
      logP_new = safe_root_without_brackets(fscvh, logP_guess, dlogP, &
                                            imax/2, imax, epslogP, epslogRho, lrpar, rpar, lipar, ipar, info)
...
!..now get the rest of the hydrogen and helium quantities
      ...
      call entropy_of_mixing(xmassh1, xmasshe4, T, P, xnh, dxnh_dlogT, dxnh_dlogP, &
                             xnh2, dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, dxnhe_dlogP, &
                             xnhep, dxnhep_dlogT, dxnhep_dlogP, smix, d_smix_dT, d_smix_dP)
...
!..sum the components
   ...
   contains
      ...
      subroutine get_values(only_densities)
         ...
         info = 0
         call interp_vals_bicub(only_densities, search_for_SCVH, &
                                den_h, ener_h, entr_h, dddt_cp_h, dddpress_ct_h, &
                                dsdt_cp_h, dsdpress_ct_h, den_he, ener_he, entr_he, &
                                dddt_cp_he, dddpress_ct_he, dsdt_cp_he, dsdpress_ct_he, xnh, &
                                dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, dxnhe_dlogP, &
                                xnhep, dxnhep_dlogT, dxnhep_dlogP, logT, logP, P, info)
   ...
   double precision function fscvh(logP, dfdlogP, lrpar, rpar, lipar, ipar, ierr)
      ...
      double precision :: logT, P, Rho, logRho_new, logRho_target, &
         dddpress, den_h, ener_h, entr_h, dddt_cp_h, dddpress_ct_h, &
         dsdt_cp_h, dsdpress_ct_h, den_he, ener_he, entr_he, &
         dddt_cp_he, dddpress_ct_he, dsdt_cp_he, dsdpress_ct_he, &
         xnh, dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, &
      ...
      call interp_vals_bicub(only_densities, search_for_SCVH, den_h, ener_h, entr_h, dddt_cp_h, dddpress_ct_h, &
                             dsdt_cp_h, dsdpress_ct_h, den_he, ener_he, entr_he, &
                             dddt_cp_he, dddpress_ct_he, dsdt_cp_he, dsdpress_ct_he, xnh, &
                             dxnh_dlogT, dxnh_dlogP, xnh2, dxnh2_dlogT, dxnh2_dlogP, xnhe, dxnhe_dlogT, dxnhe_dlogP, &
                             xnhep, dxnhep_dlogT, dxnhep_dlogP, logT, logP, P, ierr)
      if (ierr /= 0) then
         return
   ...
   subroutine entropy_of_mixing(xmassh1, xmasshe4, T, P, xnh_in, dxnh_dlogT, dxnh_dlogP, xnh2_in, &
                                dxnh2_dlogT, dxnh2_dlogP, xnhe_in, dxnhe_dlogT, dxnhe_dlogP, xnhep_in, dxnhep_dlogT, dxnhep_dlogP, &
                                smix, d_smix_dT, d_smix_dP)
      implicit none
      double precision, intent(in) :: xmassh1, xmasshe4, T, P, xnh_in, dxnh_dlogT, dxnh_dlogP, &
         xnh2_in, dxnh2_dlogT, dxnh2_dlogP, xnhe_in, dxnhe_dlogT, dxnhe_dlogP, xnhep_in, dxnhep_dlogT, dxnhep_dlogP
      ...
      double precision ::xnh, dxnh_dP, dxnh_dT, xnh2, dxnh2_dP, dxnh2_dT, xnhe, dxnhe_dP, dxnhe_dT, xnhep, dxnhep_dP, dxnhep_dT, &
         beta, num, dnum_dP, dnum_dT, denom, ddenom_dP, ddenom_dT, gama, dgama_dP, dgama_dT, delt, ddelt_dP, ddelt_dT, &
         a, da_dP, da_dT, b1, db1_dT, db1_dP, b21, db21_dT, db21_dP, &
         b22, db22_dT, db22_dP, b2, db2_dT, db2_dP, b31, &
         db31_dT, db31_dP, b32, db32_dT, db32_dP, &
         b331, db331_dT, db331_dP, b332, db332_dT, db332_dP, b33, db33_dT, db33_dP, &
         b3, db3_dT, db3_dP, b, db_dT, db_dP
