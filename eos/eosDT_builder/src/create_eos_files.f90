! ***********************************************************************
!
!   Copyright (C) 2009  Bill Paxton, Frank Timmes & The MESA Team
!
...
program create_eosDT_files
   ...
   real(dp) :: del_logT = 0.02d0, del_logQ = 0.03d0
   real(dp) :: logT_min = 2.1d0, logT_rad_max = 8.2d0, logT_no_rad_max = 7.7d0
...
contains
   ...
   subroutine Make_EoS_Files(Z_in, X_in, include_radiation)
      ...
      real(dp) :: logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, free_e, gamma1, gamma3, grad_ad, eta
...
!..other initialization
      ...
      write (io_unit, '(99(a14))') 'version', 'X', 'Z', 'num logTs', 'logT min', 'logT max', &
         'del logT', 'num logQs', 'logQ min', 'logQ max', 'del logQ'

      write (io_unit, '(i14,2f14.4,2(i10,4x,3(f14.4)))') version_number, X, Z, num_logTs, &
         logT_min, logT_max, del_logT, num_logQs, logQ_min, logQ_max, del_logQ
      ...
      do i = 1, num_logQs
         ...
         write (io_unit, '(a4,1x,3(a9,1x),7(a12,1x),1(a7,1x),1(a11),3(a9,1x),2(a9,1x))') &
            'logT', 'logPgas', 'logE', 'logS', 'chiRho', 'chiT', 'Cp', 'Cv', &
            'dE_dRho', 'dS_dT', 'dS_dRho', 'mu', 'log_free_e', 'gamma1', 'gamma3', 'grad_ad', 'eta', 'HELM'
         ...
         do j = 1, num_logTs
            ...
            Rho = 10**logRho
            info = 0
            call helm_opal_scvh(helm_only, opal_scvh_only, opal_only, scvh_only, search_for_SCVH, include_radiation, &
                                logT, logRho, T, Rho, abar, zbar, z53bar, X, Z, logPgas, logE, logS, chiRho, chiT, &
                                Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, free_e, gamma1, gamma3, grad_ad, eta, &
                                HELM_fraction, data_dir, info)
            if (info /= 0) then
               write (*, *) 'logT', logT
               write (*, *) 'logRho', logRho
