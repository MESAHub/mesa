! ***********************************************************************
!
!   Copyright (C) 2006  Bill Paxton, Frank Timmes & The MESA Team
!
...
contains
   ...
   subroutine interpolate_opal_scvh(opal_only, scvh_only, include_radiation, search_for_SCVH, &
                                    logT_in, logRho_in, temp_in, den_in, abar_in, zbar_in, X_in, Z_in, &
                                    logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, &
                                    logNe, gamma1, gamma3, grad_ad, eta, dlnPgas_dlnY, data_dir, info)
      use scvh_core
      use const_def
      implicit none
      ...
      double precision, intent(out) :: logPgas, logE, logS, chiRho, chiT, Cp, Cv, &
         dE_dRho, dS_dT, dS_dRho, mu, logNe, gamma1, gamma3, grad_ad, eta, dlnPgas_dlnY
      ...
      call do_opal_scvh(opal_only, scvh_only, include_radiation, search_for_SCVH, &
                        logT_in, logRho_in, temp_in, den_in, abar_in, zbar_in, X, Z, &
                        logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, &
                        logNe, gamma1, gamma3, grad_ad, eta, data_dir, info)
      if (info /= 0) return
   ...
   subroutine do_opal_scvh(opal_only, scvh_only, include_radiation, search_for_SCVH, &
      ...
      logical, intent(in) :: opal_only, scvh_only, search_for_SCVH, include_radiation
      double precision, intent(in) :: logT_in, logRho_in, temp_in, den_in, abar_in, zbar_in, X_in, Z_in
      double precision, intent(out) :: logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, &
         logNe, gamma1, gamma3, grad_ad, eta
      character(len=*), intent(in) ::  data_dir
      integer, intent(out) :: info ! returned = 0 if AOK
...
!..for the opal portion
      double precision :: logPgas_opal, logE_opal, logS_opal, chiRho_opal, chiT_opal, Cp_opal, Cv_opal, &
         dE_dRho_opal, dS_dT_opal, dS_dRho_opal, mu_opal, gamma1_opal, gamma3_opal, grad_ad_opal, logNe_opal

!..for the scvh portion
      double precision :: logPgas_scvh, logE_scvh, logS_scvh, chiRho_scvh, chiT_scvh, Cp_scvh, Cv_scvh, &
         dE_dRho_scvh, dS_dT_scvh, dS_dRho_scvh, mu_scvh, gamma1_scvh, gamma3_scvh, grad_ad_scvh, logNe_scvh
...
!..define the interior boundary between opal and scvh
      ...
      if (beta /= 0D0 .or. scvh_only) then
         ...
         do i = 1, 10 ! reduce logQ if necessary
            call interpolate_scvh(include_radiation, search_for_SCVH, logT_scvh, logRho_scvh, &
                                  temp_scvh, den_scvh, X, logPgas_scvh, logE_scvh, logS_scvh, &
                                  chiRho_scvh, chiT_scvh, Cp_scvh, Cv_scvh, &
                                  dE_dRho_scvh, dS_dT_scvh, dS_dRho_scvh, &
                                  mu_scvh, gamma1_scvh, gamma3_scvh, grad_ad_scvh, logNe_scvh, info)
            if (info == 0) then
               scvh_okay = .true.; exit
            end if
      ...
      if (beta == 0d0) then ! just opal
         ...
         if (.false.) then
            ...
            call blend(alfa, beta, den, temp, Prad, &
                       logPgas_opal, logPgas_scvh, logS_opal, logS_scvh, &
                       dS_dT_opal, dS_dT_scvh, dS_dRho_opal, dS_dRho_scvh, &
                       chiT_opal, chiT_scvh, chiRho_opal, chiRho_scvh, mu_opal, mu_scvh, &
                       logNe_opal, logNe_scvh, logE_opal, logE_scvh, &
                       Cv_opal, Cv_scvh, dE_dRho_opal, dE_dRho_scvh, &
   ...
   subroutine blend(alfa, beta, den, temp, Prad, logPgas_1, logPgas_2, logS_1, logS_2, &
                    ...
                    chiT_1, chiT_2, chiRho_1, chiRho_2, mu_1, mu_2, &
                    logNe_1, logNe_2, logE_1, logE_2, Cv_1, Cv_2, dE_dRho_1, dE_dRho_2, &
                    gamma1_1, gamma1_2, gamma3_1, gamma3_2, grad_ad_1, grad_ad_2, &
                    logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, mu, gamma1, gamma3, grad_ad, logNe)
      double precision, intent(in) ::   alfa, beta, den, temp, Prad, &
         logPgas_1, logPgas_2, logS_1, logS_2, dS_dT_1, dS_dT_2, dS_dRho_1, dS_dRho_2, &
         chiT_1, chiT_2, chiRho_1, chiRho_2, mu_1, mu_2, logNe_1, logNe_2, logE_1, logE_2, Cv_1, Cv_2, dE_dRho_1, dE_dRho_2, &
         gamma1_1, gamma1_2, gamma3_1, gamma3_2, grad_ad_1, grad_ad_2
      double precision, intent(out) ::   logPgas, logE, logS, chiRho, chiT, Cp, Cv, dE_dRho, dS_dT, dS_dRho, &
         mu, gamma1, gamma3, grad_ad, logNe
      double precision ::Pgas_1, Pgas_2, P_1, P_2, Pgas, P, s_1, s_2, s, &
         dP_dT, dP_dRho, e_1, e_2, e, dE_dT, dP_dT_1, dP_dT_2, dP_dRho_1, dP_dRho_2, x
