...
def write_summary(rows):
    stats = defaultdict(
        lambda: {"max_rel": 0.0, "max_abs": 0.0, "max_fd_err": 0.0, "count": 0}
    )
    for row in rows:
        ...
        stats[key]["max_fd_err"] = max(
            stats[key]["max_fd_err"], row["finite_difference_error"]
        )
        stats[key]["count"] += 1
    ...
    with SUMMARY.open("w") as f:
        ...
        f.write(
            "finite_difference uses constrained Ridders extrapolation with he4 as sink\n\n"
        )
        f.write(
            f"{'sweep':18s} {'row':20s} {'max_rel':>14s} {'max_abs':>14s} {'max_fd_err':>14s} {'n':>8s}\n"
        )
...
def plot_eos_fractions(rows, pdf):
    ...
    fig, axes = plt.subplots(
        2, 2, figsize=(6.7, 4.8), sharey=True, constrained_layout=True
    )
    axes = axes.ravel()
...
def plot_eos_region_contour(contour_rows, pdf):
    ...
    mesh = ax.pcolormesh(
        xs, ys, masked_grid(z), shading="nearest", cmap=cmap, norm=norm, rasterized=True
    )
...
def plot_partial_contours(contour_rows, pdf):
    ...
    for row_name in KEY_ROWS:
        ...
        for direction in KEY_DIRECTIONS:
            all_values.extend(
                row["raw_partial"] for row in grouped.get((row_name, direction), [])
            )
...
def plot_dfridr_contours(dfridr_rows, pdf):
    ...
    for row_name in DFRIDR_ROWS:
        ...
        for ax, direction in zip(axes, DFRIDR_DIRECTIONS):
            ...
            ax.tick_params(labelsize=7, pad=1)
        fig.suptitle(
            rf"DFRIDR composition partial check: {ROW_LABELS[row_name]}",
            y=0.985,
            fontsize=9.5,
        )
        if mesh is not None:
...
def plot_summary_heatmap(stats, pdf):
    ...
    ax.set_xticklabels(
        [SWEEP_LABELS.get(sweep, sweep) for sweep in sweeps], rotation=30, ha="right"
    )
