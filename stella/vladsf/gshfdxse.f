...
c `i' is the net charge on the ion.
     ...
     .        c_g(6,ng) * ir**2
         ...
         if ((ng .eq. 1) .or. (ng .eq. 6)) then
            ...
            sig0 = ne_shell * (1.d+00 - dfloat(zc_g(ng)) /
     .           dfloat(z))**(alpha_g(ng)) / (b_g(1,ng) + b_g(2,ng) * z
     .           + b_g(3,ng) * z**2)
         else
