...
! ***********************************************************************
      ...
      subroutine adjust_irradiation(b)
         ...
         if (b% accretion_powered_irradiation) then
            ...
            s% ctrl% irradiation_flux = min(b% max_F_irr, Lx/(4*pi*pow2(b% separation)))
            write(*,2) 'lg F_irr', s% model_number, safe_log10(s% ctrl% irradiation_flux)
            ...
            s% ctrl% irradiation_flux = b% irrad_flux_at_std_distance * &
               pow2(b% std_distance_for_irradiation/b% separation)
         end if
         s% ctrl% column_depth_for_irradiation = b% col_depth_for_eps_extra
