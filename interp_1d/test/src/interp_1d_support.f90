...
contains
   ...
   subroutine test2(increasing)
      ...
      if (show_errors) then
         do i = 2, nvals
            write (*, 2) 'x val exact err', i, xs(i), vals(i), cos(xs(i - 1)) - cos(xs(i)), vals(i) - (cos(xs(i - 1)) - cos(xs(i)))
         end do
      else
         do i = 2, nvals
   ...
   subroutine test_interp_3_to_2
      ...
      call interp_3_to_2(pdqm1, pdq00, ndqm1, ndq00, pfm1, pf00, pfp1, nf00, nfp1, 'test_interp_3_to_2', ierr)
      write (*, *) 'ierr', ierr
      write (*, 1) 'nf00', nf00
      write (*, 1) 'nfp1', nfp1
