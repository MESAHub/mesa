...
- Check that the test suite gallery tags are up to date by following :ref:`developing/test_suite:Gallery tags`, using the Python packages pinned in the top-level ``requirements-dev.txt``, and running ``python scripts/test_suite_inventory.py`` from ``$MESA_DIR``. Update ``docs/source/_ext/mesa_tags.py`` if new tags need to be grouped for the generated gallery.
