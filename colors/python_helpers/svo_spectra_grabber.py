...
class SVOSpectraGrabber:
    ...
    def _discover_spectra_ssap(self, model_name):
        ...
        try:
            ...
            response = self.session.get(self.spectra_base_url, params=params, timeout=30)
            if response.status_code == 200:
    ...
    def _discover_spectra_brute_force(self, model_name, max_fid=20000, batch_size=50):
        ...
        print(f"    Scanning for spectra (this may take a while)...")
        ...
        for start_fid in tqdm(range(1, max_fid, batch_size), desc="Scanning FIDs"):
            ...
            if consecutive_failures > max_consecutive_failures and len(spectra_info) > 0:
                print(f"    Stopping search after {consecutive_failures} consecutive failures")
                break
    ...
    def _test_spectrum_exists(self, model_name, fid):
        ...
        try:
            ...
            response = self.session.head(self.spectra_base_url, params=params, timeout=10)
            return response.status_code == 200 and response.headers.get('content-length', '0') != '0'
        except:
            return False
    ...
    def download_spectrum(self, model_name, fid, output_path):
        ...
        try:
            params = {"model": model_name, "fid": fid, "format": "ascii"}
            response = self.session.get(self.spectra_base_url, params=params, 
    ...
    def download_model_spectra(self, model_name, spectra_info):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for spectrum in spectra_info:
                ...
                future = executor.submit(self.download_spectrum, model_name, fid, output_path)
                future_to_fid[future] = (fid, filename, output_path)
            
            # Process completed downloads
            for future in tqdm(as_completed(future_to_fid), 
        ...
        if metadata_rows:
            ...
            print(f"  No spectra downloaded for {model_name}. Consider using --force-brute if the model lacks metadata.")
    ...
    def _create_lookup_table(self, output_dir, metadata_rows):
        ...
        # Write CSV
        with open(lookup_table_path, mode="w", newline="", encoding='utf-8') as csv_file:
    ...
    def select_models_interactive(self, available_models):
        ...
        try:
            ...
            selected = [available_models[i] for i in indices 
...
def main():
    ...
    parser.add_argument('--output', type=str, default='../data/stellar_models/',
