...
def normalize_to_reference(target_data, reference_data, wavelength_grid):
    ...
    """
    print(f"  Normalizing to reference model: {os.path.basename(reference_data['model_dir'])}")
    ...
    if not valid_mask.all():
        print(f"    Warning: Found {np.sum(~valid_mask)} invalid reference models, removing them")
        ...
        reference_logg = np.array(reference_data['logg'])[valid_mask]
        reference_meta = np.array(reference_data['meta'])[valid_mask]
    else:
        reference_files = reference_data['files']
        reference_teff = reference_data['teff']
        reference_logg = reference_data['logg']
        reference_meta = reference_data['meta']
    
    if len(reference_params) == 0:
        print("    Error: No valid reference models found!")
        return np.ones(len(target_data['files']))
    
    print(f"    Reference model parameter ranges:")
    print(f"      Teff: {reference_params[:, 0].min():.1f} - {reference_params[:, 0].max():.1f} K")
    print(f"      logg: {reference_params[:, 1].min():.2f} - {reference_params[:, 1].max():.2f}")
    print(f"      [M/H]: {reference_params[:, 2].min():.2f} - {reference_params[:, 2].max():.2f}")
    ...
    print(f"    Parameter ranges: Teff={teff_range:.1f}, logg={logg_range:.2f}, meta={meta_range:.2f}")
    ...
    if not np.isfinite(reference_params_norm).all():
        ...
        print(f"    Teff range: {reference_params[:, 0].min():.1f} - {reference_params[:, 0].max():.1f}")
        print(f"    logg range: {reference_params[:, 1].min():.2f} - {reference_params[:, 1].max():.2f}")
        print(f"    meta range: {reference_params[:, 2].min():.2f} - {reference_params[:, 2].max():.2f}")
        return np.ones(len(target_data['files']))
    ...
    for i, (file, teff, logg, meta) in enumerate(zip(
        ...
        if not (np.isfinite(teff) and np.isfinite(logg) and np.isfinite(meta)):
            print(f"    Warning: Invalid target parameters for {file}: Teff={teff}, logg={logg}, meta={meta}")
    ...
    print(f"    Normalization factors: median={np.median(norm_factors):.2f}, range=[{np.min(norm_factors):.2f}, {np.max(norm_factors):.2f}]")
...
def identify_reference_model(all_models_data):
    ...
    for i, model_data in enumerate(all_models_data):
        ...
        if 'kurucz' in model_name:
            print(f"Using {os.path.basename(model_data['model_dir'])} as reference model")
            return i
    
    # If no Kurucz, use the first model
    print(f"No Kurucz model found, using {os.path.basename(all_models_data[0]['model_dir'])} as reference")
    return 0
...
def visualize_parameter_space(teff_grid, logg_grid, meta_grid, source_map, 
    ...
    for idx, model_name in enumerate(model_names):
        ...
        valid_mask = np.isfinite(data['teff']) & np.isfinite(data['logg']) & np.isfinite(data['meta'])
        ...
        dist = ((valid_teff - target[0]) / 1000) ** 2 + \
        ...
        if len(dist) > 0:
            ...
            try:
                ...
                if mask.sum() > 20:
                    ax5.plot(wl[mask], wl[mask] ** 2 * fl[mask],
