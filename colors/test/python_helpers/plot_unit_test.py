...
FILTER_DIR = os.path.join(
    MESA_DIR, "data", "colors_data", "filters", "Generic", "Johnson"
) if MESA_DIR else ""
...
def parse_test_output(path: str) -> Dict[str, object]:
    ...
    with open(path, encoding="utf-8") as f:
        for raw in f:
            ...
            if section in ("group1", "group2a", "group2b", "group2c") and current_key is not None:
                parts = line.split()
                if len(parts) == 2:
...
def style_axis(ax, title: str | None = None) -> None:
    ...
    ax.minorticks_on()
    ax.grid(True, which="major", color=PG_GRID, alpha=0.18, linestyle="--", linewidth=0.6)
...
def style_secondary_axis(ax) -> None:
    ...
    ax.tick_params(axis="both", which="minor", colors=PG_FG, direction="in", top=True, right=True)
...
def main() -> None:
    ...
    sweep_handles = [
        Line2D([0], [0], color=style["color"], marker=style["marker"], linestyle="-",
               linewidth=1.6, markersize=5, label=label)
        for label, style in COLOR_INDEX_STYLES.items()
    ]
    sweep_handles.append(
        Line2D([0], [0], color=PG_FG, marker="x", linestyle="--", linewidth=1.1,
               markersize=5, label=r"$\Delta$Mag$_{bol}$")
    )
    legend = ax.legend(
    ...
    plot_color_sweep(ax, g2c, x_teff, r"$T_{eff}$ (K)", panel_tag="colour sweep :: temperature")
    ...
    ax.text(
        ...
        bbox={"boxstyle": "square,pad=0.25", "facecolor": PG_BG, "edgecolor": PG_FG, "alpha": 0.95},
    )
