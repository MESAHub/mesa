...
vega_interp = interpolate.interp1d(vega_wave, vega_flux, bounds_error=False,
                                   fill_value=0.0)
...
def do_one(f_in, vega_interp, teff):
    ...
    j_f = j['flux']
    j_f_interp = interpolate.interp1d(j_m, j_f, bounds_error=False,
                                      fill_value=0.0)
    
    j_norm = integrate.romberg(j_f_interp, j_m[0], j_m[-1], divmax=50,
                               vec_func=True, tol=1e-10, rtol=1e-10)
    ...
    def bc(t):
        constants = const(t)
        top = integrate.romberg(topfunc, j_m[0], j_m[-1], args=(t,), divmax=50,
                                vec_func=True, tol=1e-10, rtol=1e-10)
        bot = integrate.romberg(botfunc, j_m[0], j_m[-1], divmax=50,
                                vec_func=True, tol=1e-10, rtol=1e-10)
        print(t, top, bot)
