...
! ***********************************************************************
      ...
      subroutine init_opal_type1(z_in,xh_in)
         ...
         z = z_in; xh = xh_in
         if (z .ne. read_z) itimeco = 0
      end subroutine init_opal_type1


      subroutine eval_opal_type1 (z_in,xh_in,t6_in,r_in,logKap,fname,OP_file,dbg_in)
      use utils_lib, only: is_bad
      double precision, intent(in) :: z_in, xh_in, t6_in, r_in
      double precision, intent(out) :: logKap
      character (len=*) :: fname
      logical, intent(in) :: OP_file, dbg_in
...
!     >      abs(t6_in - 1d0) < 1d-6 .and. abs(r_in - 1d-6) < 1d-12)
      ...
      do i=1,10
         ...
         if (logKap < 9 .and. .not. is_bad(logKap)) exit
         ! tables have 9.999 for missing entries at small R and T.
         ! try larger temperature for same R
         logT6 = log10(t6) + 0.05
         t6 = 10**logT6
...
c               at fixed rho
      ...
      do i=1,mz
        if(abs(z-za(i)) .lt. 1.e-7 ) then ! don't need to interpolate in z
          izz=i
...
c
      ...
      do iz=m1,mfm
        ...
        if (opact .gt. 9.0) opact = 9 !write (*,'(" logK > 9.0, X=",f7.5," Z=",
!     x    f7.5," T6=",f10.5," R=",e12.4)') xh,z,t6,r
        izi=1
...
c      interpolation.
      ...
      if(slt .lt. alt(1)) slt = alt(1)
      if(slt .gt. alt(nt)) slt = alt(nt)
      if(slr .lt. alr (1)) slr = alr(1)
      if(slr .gt. alr(nre)) slr = alr(nre)
c
c
c
...
c
   ...
   11     if(ihi-ilo .gt. 1) then
      ...
      if ((mfin .eq. 1) .and. (xz(1,mzz,k1,l1) .gt. 9.)) then   ! no data
         ...
         enddo
         if (kmin <= 0) return
         if ((iadvance .eq. 0) .and. (k1 .le. kmin) .and. (slt .le. alt(kmin))) then
            k1=kmin
            if ((xz(1,mzz,kmin,l1+1) .lt. 9.) .and. ((slr+.01) .gt. alr(l1+1))) then
               l1=l1+1
               kmin=0
               k1=k1in
               do i=1,6
      ...
      if (.false.) then ! don't need this because did extrapolate during read
        do i=14,18   ! allows jagged edge at high T,rho
          if((l3s .gt. i) .and. (k3s .gt. nta(i+1))) then
            ...
            write(*,*) 'l3s .gt. i', l3s .gt. i
            write(*,*) 'k3s .gt. nta(i+1)', k3s .gt. nta(i+1)
            write(*,*) 'l3s', l3s
            write(*,*) 'i', i
            write(*,*) 'k3s', k3s
      ...
      if((zz(mg,mzin) .ne. zz(mf,mzin)) .or. (zz(mh,mzin) .ne. zz(mf,mzin))) then
         ...
         return
         if (.true. .or. abs(xh - xa(mf)) > 1d-6) then
            write(*,*) 'z', z
            write(*,*) 'za(mzz)', za(mzz)
            write(*,*) 'xh', xh
            write(*,*) 'xa(mf)', mf, xa(mf)
            write(*,*) 'l1', l1
            write(*,*) 'l1+iq', l1+iq
            do i=l1,l1+iq
               write(*,*) 'alr(i)', i, alr(i)
      ...
      if((zz(mg,mzin) .ne. zz(mf,mzin)) .or. (zz(mh,mzin) .ne. zz(mf,mzin))) then
            ...
            write(*,*) 'filename ', trim(filename)
            write(*,*)
            !write(*,*) 'zz(mf2,mzin)', mf2, zz(mf2,mzin)
            write(*,*) 'zz(mf,mzin)', mf, zz(mf,mzin)
            write(*,*) 'zz(mg,mzin)', mg, zz(mg,mzin)
...
c
   ...
   61 return
      write(*,'(" Mass fractions exceed unity")')
c                  with a return
      call mesa_error(__FILE__,__LINE__)
   62 return
...
c
c**********************************************************************
      subroutine readco(filename,OP_file)
      use utils_lib, only: mesa_error
...
c      read header
      ...
      if (OP_file) then
         imax = 239 ! OP files missing a blank line
      else
         imax = 240
      end if
      do i=1,imax
...
c
      ...
      read(2,*)
        do k=1,ntm
           ierr = 0
           read(2,'(f4.2)',advance='no',iostat=ierr) alt(k)
           if (ierr /= 0) then
            write(*,*) 'failed in opal_type1 read: alt(k)', k, alt(k)
            call mesa_error(__FILE__,__LINE__)
           end if
           !write(*,*) 'opal_type1 read: alt(k)', k, alt(k)
           read(2,'(f7.3)',advance='no',iostat=ierr) xzf(k,1)
           if (ierr /= 0) then
            write(*,*) 'failed in opal_type1 read: xzf(k,1)', k, alt(k)
            call mesa_error(__FILE__,__LINE__)
           end if
           read(2,'(f7.3)',advance='no',iostat=ierr) xzf(k,2)
           if (ierr /= 0) then
            write(*,*) 'failed in opal_type1 read: xzf(k,2)', k, alt(k)
            call mesa_error(__FILE__,__LINE__)
           end if
           do l=3,nrm
            ierr = 0
            read(2,'(f7.3)',advance='no',iostat=ierr) xzf(k,l)
            if (ierr /= 0) then ! extrapolate to fill in the missing entries
              do ll=l,nrm
               xzf(k,ll) = (3*xzf(k,ll-1) - xzf(k,ll-2))*0.5d0
              end do
              !write(*,*) 'extend data', alt(k), l
              exit
            end if
           end do
           if (ierr == 0) read(2,*) 
...
c***********************************************************************
     ...
     x .04,.06,.08,.1/
      ...
      data (nta(i),i=1,nrm)/19*70/  !    << extrapolate to fill
