...
# This syncs up the history and profile files in each test case with the
# default versions
...
def update(default, test_suite, special_cases=None, debug=False):
    ...
    for file in test_suite:
        ...
        with open(file, "w") as f:
            for line in lines:
                # For each line in the default file we check if it was
                # enabled in the test case version
                match = r.match(line)
                if match:
                    if match.group(1) in test_case:
                        # If enabled make sure it stays enabled when we
                        # write the file back out
                        groups = match_uncomment.match(line)
                        # We want the first ! before the option but not !
                        # after the option
...
def update_history():
    ...
    default = os.path.join(MESA_DIR, "star", "defaults",
                           "history_columns.list")
    
    # Special case things that are name number
    special_cases = set(
        ['burning_regions', 'mixing_regions', 'mix_relr_regions',
         'burn_relr_regions'])
