...
RSP2 adds cell $\delta w$ and face $\delta Y_{\rm face}$, with $e_t=w^2$.
...
Use $e_t=w^2$, $C=S-D-D_r$, turbulent luminosity $L_t$, and turbulent
pressure $P_t$. The source and flux formulas are in Appendix~\ref{app:rsp2}.
\begin{rowpair}{LnaDynamic}
\begin{equation}
  \frac{\dd w_k^2}{\dd t}
    =C_k+Eq_k-\frac{L_{t,k}-L_{t,k+1}}{\Delta m_k}
       -\frac{P_{t,k}}{\Delta m_k}\dot{\mathcal V}_k.
\end{equation}
\begin{linearized}{dynamic row}
\begin{equation}
  \delta C_k-\frac{\delta L_{t,k}-\delta L_{t,k+1}}{\Delta m_k}
      -\frac{P_{t,k}}{\Delta m_k}\delta\dot{\mathcal V}_k
    =\sigma\,2w_k\delta w_k.
  \label{eq:rsp2energy}
\end{equation}
\end{linearized}
\end{rowpair}
\textit{Code:} \code{assemble_rsp2_w_row} and
\code{rsp2_turbulent_energy_rhs_for_star_LNA}.
Here $\delta C=\delta S-\delta D-\delta D_r$ and
$\delta\dot{\mathcal V}_k=\mathcal A_k\delta v_{f,k}
-\mathcal A_{k+1}\delta v_{f,k+1}$, with fixed physical inner velocity.
The compression coefficient is the background $P_t$, because
$\dot{\mathcal V}_0=0$. Viscous heating is quadratic about a static
background, so $\delta Eq=0$. Forced nonturbulent cells use $\delta w=0$.
In the three equation model, $C=S-D$: radiative cooling acts in the entropy
moments, so the direct $D_r$ term is absent. Both gas energy forms retain
their existing turbulent storage term; no additional exchange term is added.
\end{equationblock}
...
The cell variable remains $w$, and the face turbulent energy is
$e_{t,f}=a_fw_k^2+(1-a_f)w_{k-1}^2$. The face entropy flux is
...
adjacent turbulent energies and both moments zero use the same zero branch.
A face beside finite turbulent energy retains both evolution rows.
LNA fixes $\delta w=0$ in a fully dormant cell. At zero represented cell
...
Constraining a cell does not constrain its active bounding moment faces.
...
no Hp row or independent $\delta H_P$. RSP2 cell w and TDC face convective
...
RSP2 stores $w$ in cells. Its face value is
\begin{equation}
 w_f=a_fw_k+(1-a_f)w_{k-1},\qquad
 a_f=\begin{cases}
 \Delta m_{k-1}/(\Delta m_{k-1}+\Delta m_k),&\text{mass interpolation},\\
 1/2,&\text{otherwise}.
 \end{cases}
\end{equation}
These are the weights in \path{get_RSP2_alfa_beta_face_weights}.
...
harmonic lengths are selected. PII is zero at $k=1,N$ and in the selected
forced-nonturbulent face region.
...
\begin{align}
 ...
 \label{eq:rsp2lc}\\
 L_{t,k}&=-\alpha_t\mathcal A_k^2(\rho^2)_f\Lambda_fw_f
                  \frac{w_{k-1}^2-w_k^2}{\code{dm_bar(k)}}.
\end{align}
...
\subsection{Cell source and damping}

Using the actual cell EOS quantities, the buoyant source is
\begin{equation}
 \mathrm{Source}_k=(w_k+w_{\rm seed})
 \left\langle\frac{\mathrm{PII}}{Hp}\right\rangle_k
 \frac{P_{{\rm eos},k}\chi_{T,k}}{\rho_k\chi_{\rho,k}C_{P,k}},
 \label{eq:rsp2source}
\end{equation}
where the bracket is half the sum at the two bounding faces for $k<N$,
and the single available ratio at $k=N$. The source uses cell w after
averaging the face PII/Hp. It does not average $w\,\mathrm{PII}/Hp$
from faces; with zero seed it vanishes when the cell w is zero, even if an
adjacent cell carries convection. With
$x_D=(8/3)\sqrt{2/3}$ and $x_R=2\sqrt3$, the damping terms are
\begin{align}
 D_k&=\alpha_d x_D\frac{w_k^3}{\Lambda_{\mathrm{cell},k}},
 \label{eq:rsp2damp}\\
 D_{r,k}&=4\sigma_{\rm SB}(\alpha_r x_R)^2
 \frac{w_k^2T_k^3}{\rho_k^2 C_{P,k}\kappa_k\Lambda_{\mathrm{cell},k}^2}.
 \label{eq:rsp2raddamp}
\end{align}
These are \code{compute_Source}, \code{compute_D}, and \code{compute_Dr}.
The LNA coupling wrapper uses those routines and returns
$C=\mathrm{COUPL}=\mathrm{Source}-D-D_r$.
The former independently calculated cell Hp is not used in damping.
...
\code{hydro_rsp2.f90}.
Only the scale-height and mixing-length functions are shared. This keeps
RSP2 cell w separate from TDC face $v_c/\sqrt{2/3}$.
...
stress velocity is $w_k$ in RSP2 and
$(v_{c,k}+v_{c,k+1})/(2\sqrt{2/3})$ in TDC. Only half the available TDC face
value is used at the innermost cell. The cell stress is
\begin{equation}
...
For \code{u_flag}, RSP2 interpolates its cell w to the face using its own
face weights. TDC uses its stored face $v_c/\sqrt{2/3}$. The face stress is
\begin{equation}
...
$\Delta m_N/2$. Its turbulent velocity is the innermost cell w for RSP2,
...
same-stress cancellation then has a lag difference. RSP2 uses current cell w
...
\end{equation}
The cell source reconstructs the correlation with the local cell velocity:
\begin{equation}
 S_k=\frac12\sum_{f=k,k+1}
 \frac{w_k}{\sqrt{e_{t,f}}}
 \left(-\frac{\partial_r P}{\rho}\right)_f
 \frac{\chi_{T,f}}{\chi_{\rho,f}C_{P,f}}\mathrm{Pi}_f .
\end{equation}
Forced faces contribute zero. A zero energy face with zero Pi contributes
zero without division; nonzero Pi at zero face energy is invalid.
The reconstruction makes the source vanish in an empty cell without
removing moment evolution at its interface with a turbulent neighbor.
...
with the model: the default model loses cell turbulent energy through
